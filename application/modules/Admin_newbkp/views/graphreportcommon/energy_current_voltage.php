...
var data=<?php echo json_encode($current['PF']);?>;
    ...
    for(var k=0;k<data.length;k++){
        var meter=data[k]["meter"];
         ...
         var time2;
         var time3;
        for(var i=0;i<data[k]["c1_data"].length;i++){
        time = (new Date(data[k]["c1_data"][i]["time"])).getTime()+(5*60+30)*60000;

        xdata.push([time, parseFloat(data[k]["c1_data"][i]["CurReading"])]);
        }
        for(var i=0;i<data[k]["c2_data"].length;i++){
            time2 = (new Date(data[k]["c2_data"][i]["time"])).getTime()+(5*60+30)*60000;
        xdata2.push([time2, parseFloat(data[k]["c2_data"][i]["CurReading"])]);
        }
        for(var i=0;i<data[k]["c2_data"].length;i++){
            time3 = (new Date(data[k]["c3_data"][i]["time"])).getTime()+(5*60+30)*60000;
       xdata3.push([time3, parseFloat(data[k]["c3_data"][i]["CurReading"])]);
       }
