...
class WashroomDemoData extends CI_Controller
{
    ...
    function getWashroomDashboardData(){
        ...
        if($date==$today){
            foreach ($branches as $row) {
                $wash_results[$i]=$this->Demo_model->getSodexoReport1($date,$row['location'],$row['branch'],$row['stationid'],$row['table_name_live'],$row['built_since'],$row['managed_by'],$row['operator_name'],$row['operator_contact'],$time); 
                $i++;
            }
        }else{
            foreach ($branches as $row) {
                $wash_results[$i]=$this->Demo_model->getSodexoReport1($date,$row['location'],$row['branch'],$row['stationid'],$row['table_name'],$row['built_since'],$row['managed_by'],$row['operator_name'],$row['operator_contact'],$time); 
                $i++;
            }
        }
