...
//Odour Male Left
    ...
    function getOudorLevel($data){
        $resultArray=array();
        $date = "'".$data['fromdate']."'";
        $queryleft="SELECT TxnTime as ToTime,CurReading FROM hardware_station_consumption_data_wr_collector where StationId=2022000112 and LineConnected='Odour Male Left' and TxnDate=".$date." order by TxnTime ASC";
         

         $queryright="SELECT TxnTime as ToTime,CurReading FROM hardware_station_consumption_data_wr_collector where StationId=2022000112 and LineConnected='Odour Male Right' and TxnDate=".$date." order by TxnTime ASC";
    ...
    $resultArray['rightxdata']=$xdatahmd;
    $resultArray['rightydata']=$ydatahmd;
    $insQuery="INSERT INTO odourtotaldata(`StationId`,`leftxdata`,`leftydata`,`rightxdata`,`rightydata`,`Date`)VALUES(2022000112,'".json_encode($resultArray['leftxdata'])."','".json_encode($resultArray['leftydata'])."','".json_encode($resultArray['rightxdata'])."','".json_encode($resultArray['rightydata'])."',".$date.")";
           //echo $insQuery;
    ...
    $jani2=0;
        ...
        if(count($dataj2times)>0){
            for ($janit=0; $janit < count($dataj2times); $janit++) { 
               $qleft="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj2times[$janit]->FromTime."' and ToTime='".$dataj2times[$janit]->ToTime."'";
                $qright="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj2times[$janit]->FromTime."' and ToTime='".$dataj2times[$janit]->ToTime."'";
                ...
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj2times[$janit]->ToTime."' ";
                $qrightin="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj2times[$janit]->ToTime."'";
        ...
        if(count($dataj1times)>0){
            for ($janit=0; $janit < count($dataj1times); $janit++) { 
               $qleft="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj1times[$janit]->FromTime."' and ToTime='".$dataj1times[$janit]->ToTime."'";
                $qright="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj1times[$janit]->FromTime."' and ToTime='".$dataj1times[$janit]->ToTime."'";
                 $qleftresult = $this->db->query($qleft)->result();
                 $qrightresult = $this->db->query($qright)->result();
                if($qleftresult[0]->Consumption<20 || $qrightresult[0]->Consumption<20){
                    ...
                    $qleftin="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj1times[$janit]->ToTime."' ";
                $qrightin="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate=".$date." and  FromTime='".$dataj1times[$janit]->ToTime."'";
                 $qleftresultin = $this->db->query($qleftin)->result();
                 $qrightresultin = $this->db->query($qrightin)->result();
                if($qleftresult[0]->Consumption<=$qleftresultin[0]->Consumption||$qrightresult[0]->Consumption<=$qrightresultin[0]->Consumption){
    ...
    //     $queryFeedback = "SELECT ToiletName,HandTowel,DustbinFull,Toiletroll,NoDustbin,FloorDry,FloorWet,HandSoap,HandTowelTime,ToiletrollTime,DustbinFullTime,NoDustbinTime,FloorDryTime,FloorWetTime,HandSoapTime FROM ToiletData where ToiletId='2022000112' and ToiletName='".$toiletId."'";
...
function getOdourDataFromDb($date){
    $dataquery="select leftxdata,leftydata,rightxdata,rightydata  from odourtotaldata where Date='".$date."' and StationId=2022000112";
    //echo $dataquery;
       $data = $this->db->query($dataquery)->result();
       if (count($data)>0) {
...
function getFootfallDataFromDb($date){
    $dataquery="select xdata,ydata  from footfalltotaldata where Date='".$date."' and StationId=2022000112";
    //echo $dataquery;
       $data = $this->db->query($dataquery)->result();
       if (count($data)>0) {
          $fulldata['xdata']=json_decode($data[0]->xdata);
    ...
    if($data['fromdate'] == $data['todate'] )
    {
     ...
     // echo  count($dataj1times);die();  
        ...
        if(count($dataj2times)>0){
            for ($janit=0; $janit < count($dataj2times); $janit++) { 
               $qleft="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj2times[$janit]->FromTime."' and ToTime='".$dataj2times[$janit]->ToTime."'";
                $qright="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj2times[$janit]->FromTime."' and ToTime='".$dataj2times[$janit]->ToTime."'";
                 $qleftresult = $this->db->query($qleft)->result();
                 $qrightresult = $this->db->query($qright)->result();
                if($qleftresult[0]->Consumption<20 || $qrightresult[0]->Consumption<20){
                    $j2vfd+=$dataj2times[$janit]->Consumption;
                    ...
                    $qleftin="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj2times[$janit]->ToTime."' ";
                $qrightin="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj2times[$janit]->ToTime."'";
                 $qleftresultin = $this->db->query($qleftin)->result();
                 $qrightresultin = $this->db->query($qrightin)->result();
                if($qleftresult[0]->Consumption<=$qleftresultin[0]->Consumption||$qrightresult[0]->Consumption<=$qrightresultin[0]->Consumption){
        ...
        if(count($dataj1times)>0){
            for ($janit=0; $janit < count($dataj1times); $janit++) { 
               $qleft="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj1times[$janit]->FromTime."' and ToTime='".$dataj1times[$janit]->ToTime."'";
                $qright="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj1times[$janit]->FromTime."' and ToTime='".$dataj1times[$janit]->ToTime."'";
                 $qleftresult = $this->db->query($qleft)->result();
                 $qrightresult = $this->db->query($qright)->result();
                if($qleftresult[0]->Consumption<20 || $qrightresult[0]->Consumption<20){
                    $j1vfd+=$dataj1times[$janit]->Consumption;
                    ...
                    $qleftin="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Left' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj1times[$janit]->ToTime."' ";
                $qrightin="SELECT Consumption FROM hardware_station_consumption_data_wr_collector where
                LineConnected='Odour Male Right' and StationId=2022000112 and TxnDate='".$datesarray[$k]."' and  FromTime='".$dataj1times[$janit]->ToTime."'";
                 $qleftresultin = $this->db->query($qleftin)->result();
                 $qrightresultin = $this->db->query($qrightin)->result();
                if($qleftresult[0]->Consumption<=$qleftresultin[0]->Consumption||$qrightresult[0]->Consumption<=$qrightresultin[0]->Consumption){
    ...
    // $today=date("Y-m-d");
       $today="2020-11-04";
        ...
        { 
            ...
            $querygood="SELECT SUM(Consumption) as Consumption FROM hardware_station_consumption_data_wr_collector where LineConnected='FB Good' and TxnDate=".$date."   and FromTime BETWEEN '".$from."' AND '".$to."'";
            $queryavg="SELECT SUM(Consumption) as Consumption FROM hardware_station_consumption_data_wr_collector where LineConnected='FB Avg' and TxnDate=".$date."   and FromTime BETWEEN '".$from."' AND '".$to."'";
             $querypoor="SELECT SUM(Consumption) as Consumption FROM hardware_station_consumption_data_wr_collector where LineConnected='FB Poor' and TxnDate=".$date."   and FromTime BETWEEN '".$from."' AND '".$to."'"; 
            // echo $querygood;die();
...
function getFeedbackDataDayLivecreport($date){
    ...
    if($data['fromdate'] == $data['todate'] )
    {
        ...
        // echo "string";die();
             ...
             $querygood="SELECT SUM(Consumption) as Consumption FROM hardware_station_consumption_data_wr_collector where LineConnected='FB Good' and TxnDate='".$datesarray[$k]."'";
            $queryavg="SELECT SUM(Consumption) as Consumption FROM hardware_station_consumption_data_wr_collector where LineConnected='FB Avg' and TxnDate='".$datesarray[$k]."'";
             $querypoor="SELECT SUM(Consumption) as Consumption FROM hardware_station_consumption_data_wr_collector where LineConnected='FB Poor' and TxnDate='".$datesarray[$k]."'"; 
            $gooddata = $this->db->query($querygood)->result();
            $avgdata = $this->db->query($queryavg)->result();
            $poordata = $this->db->query($querypoor)->result();
    ...
    function getsodexoDataLivecreport($date){
      ...
      for ($fl = 0; $fl < count($fulldata); $fl++) 
          ...
          $fulldata1['xdata']=$hoursxaxisarray;
          $fulldata1['ydata']=$footfallyaxisarray;
          $insQuery="INSERT INTO footfalltotaldata(`StationId`,`xdata`,`ydata`,`Date`)VALUES(2022000112,'".json_encode($fulldata1['xdata'])."','".json_encode($fulldata1['ydata'])."',".$date1.")";
          // echo $insQuery;
          $this->db->query($insQuery);
         // echo json_encode($sam);
    ...
    function getOdourRightLevelsMail($date){
         ...
         $result=array();
         $query60to120="SELECT CurReading,FromTime FROM hardware_station_consumption_data_wr_collector where TxnDate='".$date."' and StationId=2022000112 and LineConnected='Odour Male Right' and CurReading BETWEEN 60 AND 120 order by FromTime";
         $query120="SELECT CurReading,FromTime FROM hardware_station_consumption_data_wr_collector where TxnDate='".$date."' and StationId=2022000112 and LineConnected='Odour Male Right' and CurReading >120 order by FromTime";
    ...
    function getOdourLeftLevelsMail($date){
         ...
         $result=array();
         $query60to120="SELECT CurReading,FromTime FROM hardware_station_consumption_data_wr_collector where TxnDate='".$date."' and StationId=2022000112 and LineConnected='Odour Male Left' and CurReading BETWEEN 60 AND 120 order by FromTime";
         $query120="SELECT CurReading,FromTime FROM hardware_station_consumption_data_wr_collector where TxnDate='".$date."' and StationId=2022000112 and LineConnected='Odour Male Left' and CurReading >120 order by FromTime";
         $data60to120 = $this->db->query($query60to120)->result();
         $data120 = $this->db->query($query120)->result();
         $result[0]=$data60to120;
         $result[1]=$data120;
    ...
    function getOdourLeftDataMail($date){
      ...
      $query="SELECT ToTime,CurReading FROM hardware_station_consumption_data_wr_collector where StationId=2022000112 and LineConnected='Odour Male Left' and TxnDate='".$date."'";
      //echo $query;
       $data = $this->db->query($query)->result();
       $fulldata=array();
       $resultodour=array();
...
//print_r($resultodourfulldata);
    ...
    function getOdourRightDataMail($date){
      ...
      $query="SELECT ToTime,CurReading FROM hardware_station_consumption_data_wr_collector where StationId=2022000112 and LineConnected='Odour Male Right' and TxnDate='".$date."'";
      //echo $query;
       $data = $this->db->query($query)->result();
       $fulldata=array();
       $resultodour=array();
...
function getFootfallReport($date){  
                    ...
                    $tdate=date("Y-m-d");
                    if($date>=$tdate){
                        $query="select round(SUM(Consumption)/2) as Consumption FROM hardware_station_consumption_data_wr_collector where StationId=2022000112 and TxnDate='".$date."' and LineConnected='Footfall Male' ";
                     $data = $this->db->query($query)->result();
                     $querysms="select COUNT(*) as sms FROM OdouSmsData where StationId=2022000112 and TxnDate='".$date."'  ";
...
function getFootfallReportDb($date){
       $dataquery="select FootFallCount,smscount from samsungfootfallwithsms where Date='".$date."' and StationId=2022000112";
       $data = $this->db->query($dataquery)->result();
       if (count($data)>0) {
        ...
        // echo "string";die();
               $date_from = strtotime($data['fromdate']); 
                ...
                for ($k=0; $k < count($datesarray); $k++) { 
                    ...
                    $query="select round(SUM(Consumption)/2) as Consumption FROM hardware_station_consumption_data_wr_collector where StationId=2022000112 and TxnDate='".$datesarray[$k]."' and LineConnected='Footfall Male' ";
                     $data = $this->db->query($query)->result();
                     if(isset($data[0]->Consumption)){
                 $footfall=$data[0]->Consumption;
                    }else{
