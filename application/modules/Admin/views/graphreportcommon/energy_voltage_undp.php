...
<?php for($i=0;$i<count($voltage['unsg']);$i++){ ?>
    <div class="GrphMnHldr WtrLvl">
        <div class="GrphDv" id="container_voltage_unsg<?php echo $i; ?>"></div>
    </div>
    
<?php } ?>
<span class="SctnTtl">Annexe Building</span>
<?php for($i=0;$i<count($voltage['unab']);$i++){ ?>
    <div class="GrphMnHldr WtrLvl">
        <div class="GrphDv" id="container_voltage_unab<?php echo $i; ?>"></div>
    </div>
...
<script>
    ...
    var energy_data_c3=<?php echo json_encode($voltage['unew']);?>;
    
    for(var k=0;k<energy_data_c3.length;k++){
        var meter=energy_data_c3[k]["meter"];
         var contain="container_voltage_unew"+k;
         var xdata=[];
         var xdata2=[];
         var xdata3=[];
         var time;
         var time2;
         var time3;
        for(var i=0;i<energy_data_c3[k]["v1_data"].length;i++){
        time = (new Date(energy_data_c3[k]["v1_data"][i]["time"])).getTime()+(5*60+30)*60000;

        xdata.push([time, parseFloat(energy_data_c3[k]["v1_data"][i]["CurReading"])]);
        }
        for(var i=0;i<energy_data_c3[k]["v2_data"].length;i++){
            time2 = (new Date(energy_data_c3[k]["v2_data"][i]["time"])).getTime()+(5*60+30)*60000;
        xdata2.push([time2, parseFloat(energy_data_c3[k]["v2_data"][i]["CurReading"])]);
        }
        for(var i=0;i<energy_data_c3[k]["v3_data"].length;i++){
            time3 = (new Date(energy_data_c3[k]["v3_data"][i]["time"])).getTime()+(5*60+30)*60000;
       xdata3.push([time3, parseFloat(energy_data_c3[k]["v3_data"][i]["CurReading"])]);
       }
    ...
    var energy_data_c5=<?php echo json_encode($voltage['unww']);?>;
    
    for(var k=0;k<energy_data_c5.length;k++){
        var meter=energy_data_c5[k]["meter"];
         var contain="container_voltage_unww"+k;
         var xdata=[];
         var xdata2=[];
         var xdata3=[];
         var time;
         var time2;
         var time3;
        for(var i=0;i<energy_data_c5[k]["v1_data"].length;i++){
        time = (new Date(energy_data_c5[k]["v1_data"][i]["time"])).getTime()+(5*60+30)*60000;

        xdata.push([time, parseFloat(energy_data_c5[k]["v1_data"][i]["CurReading"])]);
        }
        for(var i=0;i<energy_data_c5[k]["v2_data"].length;i++){
            time2 = (new Date(energy_data_c5[k]["v2_data"][i]["time"])).getTime()+(5*60+30)*60000;
        xdata2.push([time2, parseFloat(energy_data_c5[k]["v2_data"][i]["CurReading"])]);
        }
        for(var i=0;i<energy_data_c5[k]["v3_data"].length;i++){
            time3 = (new Date(energy_data_c5[k]["v3_data"][i]["time"])).getTime()+(5*60+30)*60000;
       xdata3.push([time3, parseFloat(energy_data_c5[k]["v3_data"][i]["CurReading"])]);
       }
    ...
    var energy_data_c6=<?php echo json_encode($voltage['unsg']);?>;
    
    for(var k=0;k<energy_data_c6.length;k++){
        var meter=energy_data_c6[k]["meter"];
         var contain="container_voltage_unsg"+k;
         var xdata=[];
         var xdata2=[];
         var xdata3=[];
         var time;
         var time2;
         var time3;
        for(var i=0;i<energy_data_c6[k]["v1_data"].length;i++){
        time = (new Date(energy_data_c6[k]["v1_data"][i]["time"])).getTime()+(5*60+30)*60000;

        xdata.push([time, parseFloat(energy_data_c6[k]["v1_data"][i]["CurReading"])]);
        }
        for(var i=0;i<energy_data_c6[k]["v2_data"].length;i++){
            time2 = (new Date(energy_data_c6[k]["v2_data"][i]["time"])).getTime()+(5*60+30)*60000;
        xdata2.push([time2, parseFloat(energy_data_c6[k]["v2_data"][i]["CurReading"])]);
        }
        for(var i=0;i<energy_data_c6[k]["v3_data"].length;i++){
            time3 = (new Date(energy_data_c6[k]["v3_data"][i]["time"])).getTime()+(5*60+30)*60000;
       xdata3.push([time3, parseFloat(energy_data_c6[k]["v3_data"][i]["CurReading"])]);
       }
