<?php
class Api_data_model_rainbow extends CI_Model{
	...
	function get_devices_list_konda($cat_id){
		$this->db->select('hardware_device');
        $this->db->from('hardware');        
        
		$this->db->where('client_id',29);  
		$this->db->where('status',1);
		$this->db->where('hardware_category',$cat_id);
        $this->db->group_by('hardware_device');
        $res = $this->db->get()->result_array();        
	...
	function get_hardwares_device_data_energy_meters_kondapur($data){
		$station_id=$data['station_id'];
		$hardware_name=$data['api_name'];
		$lineconnected=$data['LineConnected'];
		$todayDate=date("Y-m-d");
		// $todayDate="2022-03-30";
		$table_name=$this->get_table_name($station_id);
		
		$meter_list=$this->get_energymeter_list_kondapur($table_name);
		$yesterDay = date('Y-m-d',strtotime("-1 days"));
		...
		foreach($meter_list as $meters){
			...
			//echo $enquery;die();
				$consdata = $this->db->query($enquery)->result_array();
			   $today_cons=round($consdata[0]['cons'],2);
				$enquery_kw="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
			     $kwdata = $this->db->query($enquery_kw)->result_array();

				 $enquery_current1="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
			     $c1_data = $this->db->query($enquery_current1)->result_array();

				 $enquery_current2="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
			     $c2_data = $this->db->query($enquery_current2)->result_array();

				 $enquery_current3="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
			     $c3_data = $this->db->query($enquery_current3)->result_array();


				 $enquery_volt1="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
			     $v1_data = $this->db->query($enquery_volt1)->result_array();

				 $enquery_volt2="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
			     $v2_data = $this->db->query($enquery_volt2)->result_array();

				 $enquery_volt3="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
			     $v3_data = $this->db->query($enquery_volt3)->result_array();


				 $enquery_kva="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='KVA'	ORDER BY TxnTime desc limit 1";
			     $kva_data = $this->db->query($enquery_kva)->result_array();

				 $enquery_kvah="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='KVAH'	ORDER BY TxnTime desc limit 1";
			     $kvah_data = $this->db->query($enquery_kvah)->result_array();
				 ...
				 for ($k=0; $k < count($datesarray_month); $k++)
						 { 
							$month_check=$this->chech_energy_consumotion($meters['UtilityName'],$datesarray_month[$k]);
							if(count($month_check)==1){
								$monthly_cons+=(float)$month_check[0]['consumption'];
							}else{
				...
				$enquery_pf="SELECT Consumption as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='PF'	ORDER BY TxnTime desc limit 1";
			//echo $enquery;die();kW
				
				$pfdata = $this->db->query($enquery_pf)->result_array();
				$resdata[$i]['pf']=$pfdata[0]['cons'];
			if($meters['UtilityName']=='Vrf Indoor'){
				$resdata[$i]['meter']='VRF Indoor';
			}else{
				$resdata[$i]['meter']='VRF Outdoor';;
			}
	...
	foreach($meter_list as $meters){
		...
		for($t=0;$t<count($datesarray);$t++){
			if($datesarray[$t]>=date('Y-m-d')){
				...
				$check=$this->check_water_level_data_mum($meters['UtilityName'],$meters['LocationName'],$datesarray[$t]);
				if(count($check)==1){
...
//echo json_encode($resdata);die();
   return $resdata;
	...
	function get_hardwares_device_data_waterlevelmeter($data){
		...
		$locationName=$data['LocationName'];
		$capacity=$data['capacity'];
		// if($table_name=='hardware_station_consumption_data_rainbow_vikrampuri'){
		// 	$todayDate="2022-04-01";
		// }else{
		// 	$todayDate=date("Y-m-d");
		// }
		...
		$resdata['meter']='OHT - Domestic Water';
...
// echo json_encode($datesarray_month);die();
			...
			if($fromtime=='Select FromTime' || $fromtime=="NA"){
				for ($k=0; $k < count($datesarray); $k++)
					{ 
						...
						$check=$this->chech_dg_running($hardware_name,$datesarray[$k]);
						if(count($check)==1){
	...
	function getDgHardwareData($table_name,$date,$hardware_name){
		             ...
		             $check=$this->chech_dg_running($hardware_name,$date);
						if(count($check)==1){
							//$resdata['consolidate'][$k]['status']=$check[0]['dg_name'];
	...
	function getfirePressureData1($table_name,$fromdate,$todate,$station_id,$datesarray){
		...
		for($t=0;$t<count($datesarray);$t++){
			...
			if(count($check)==1){
				...
				if($datesarray[$t]>=date('Y-m-d')){
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name where UtilityName='Old Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0068' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate = '".$datesarray[$t]."'   ORDER BY TxnDate ASC,TxnTime ASC ";
					//echo $pressure;
					$pressuredata = $this->db->query($pressure)->result();
	...
	function getfirePressureData($table_name,$fromdate,$todate,$station_id,$datesarray){
		...
		for($t=0;$t<count($datesarray);$t++){
			...
			if(count($check)==1){
				...
				if($datesarray[$t]>=date('Y-m-d')){
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name where UtilityName='New Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0069' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate = '".$datesarray[$t]."'   ORDER BY TxnDate ASC,TxnTime ASC ";
					//echo $pressure;
					$pressuredata = $this->db->query($pressure)->result();
					$pressuredata2=array_merge($pressuredata2,$pressuredata);
				}else{
...
// echo json_encode($firepumpList);die();
	...
	function get_hardwares_device_data_energymeter_report_tbl($data,$fromdate,$todate,$fromtime,$totime){
		$station_id=$data['station_id'];
		$hardware_name=$data['api_name'];
		$lineconnected=$data['LineConnected'];
		$utilityName=$data['UtilityName'];
		...
		foreach($meter_list as $meters){
							...
							//$resdata[$i][$k]['meter']=$meters['LocationName'];
								...
								if($meters['LocationName']=="AFS project-C"){ 
									$resdata['consolidate'][$k][$i]['meter']="Container office";
								}else if($meters['LocationName']=="B4 Building"){
									$resdata['consolidate'][$k][$i]['meter']="A4 Building";
								}else if($meters['LocationName']=="Mains"){
									$resdata['consolidate'][$k][$i]['meter']="Main I/C (EB)";
								}else if($meters['LocationName']=="Fire Fighting"){
									$resdata['consolidate'][$k][$i]['meter']="Fire pump panel";
								}else if($meters['LocationName']=="LDB Pump"){
									$resdata['consolidate'][$k][$i]['meter']="LDB (Pump room)";
								}else if($meters['LocationName']=='PDB pump'){
									$resdata['consolidate'][$k][$i]['meter']='PDB Panel';
								}else if($meters['LocationName']=="LDB Street"){
									$resdata['consolidate'][$k][$i]['meter']="LDB (Street light panel)";
								}else{
									$resdata['consolidate'][$k][$i]['meter']=$meters['LocationName'];
								}
								$queryconsutoday="SELECT SUM(Consumption) as cons FROM $table_name WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$datesarray[$k]."' AND LineConnected='kWh'	ORDER BY TxnTime";
								//echo $queryconsutoday;die();
								
								$datacontoday = $this->db->query($queryconsutoday)->result();
								$resdata['consolidate'][$k][$i]['consumption']=(float)$datacontoday[0]->cons;
					
								$resdata['consolidate'][$k][$i]['date']=$datesarray[$k];
	...
	function get_hardwares_device_data_energymeter_report_kondapur_tbl($data,$fromdate,$todate,$fromtime,$totime){
		$station_id=$data['station_id'];
		$hardware_name=$data['api_name'];
		$lineconnected=$data['LineConnected'];
		$utilityName=$data['UtilityName'];
		...
		foreach($meter_list as $meters){
								...
								$resdata['consolidate'][$k][$i]['meter']=$meters['UtilityName'];
								$queryconsutoday="SELECT SUM(Consumption) as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$datesarray[$k]."' AND LineConnected='kWh'	ORDER BY TxnTime";
	...
	function get_hardwares_device_data_energymeter_report_kondapur($data,$fromdate,$todate,$fromtime,$totime){
		$station_id=$data['station_id'];
		$hardware_name=$data['api_name'];
		$lineconnected=$data['LineConnected'];
		$utilityName=$data['UtilityName'];
		...
		foreach($meter_list as $meters){
					...
					for ($k=0; $k < count($datesarray); $k++)
				{ 
					$check=$this->chech_energy_consumotion($meters['UtilityName'],$datesarray[$k]);
					if(count($check)==1){
						$resdata['consolidate'][$i][$k]['meter']=$check[0]['meter_name'];
						$resdata['consolidate'][$i][$k]['consumption']=$check[0]['consumption'];		
						$resdata['consolidate'][$i][$k]['date']=$check[0]['report_date'];
						...
						if($datesarray[$k]>=date('Y-m-d')){
								...
								$resdata['consolidate'][$i][$k]['meter']=$meters['UtilityName'];
								$queryconsutoday="SELECT SUM(Consumption) as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$datesarray[$k]."' AND LineConnected='kWh'	ORDER BY TxnTime";
								...
								$resdata['consolidate'][$i][$k]['date']=$datesarray[$k];
									...
									$energy_cons_query=array(
										'location_name'=>$meters['UtilityName'],
										'meter_serial'=>'',
										'report_date'=>$datesarray[$k],
										'created_date'=>date('Y-m-d H:i:s'),
										'updated_date'=>date('Y-m-d H:i:s'),
	...
	$i=0;
		foreach($meter_list as $meters){
			...
			$resdata['summery'][$i]['meter']=$meters['UtilityName'];
			...
			$hldata1=array();
			for($t=0;$t<$totaldays;$t++){
				$check=$this->chech_energy_consumotion($meters['UtilityName'],$datesarray[$t]);
				if(count($check)==1){
					$tcons+=$check[0]['consumption'];
					$hldata[$t]['con']=$check[0]['consumption'];
					$hldata[$t]['date']=$check[0]['report_date'];
					...
					//die();
					$queryconsucum="SELECT SUM(Consumption) as cons FROM $table_name WHERE `UtilityName`='".$meters['UtilityName']."' AND `TxnDate`='".$datesarray[$t]."' AND LineConnected='kWh' ";
					$dataconcum = $this->db->query($queryconsucum)->result_array();	
					$tcons+=$dataconcum[0]['cons'];
					$hldata[$t]['con']=$dataconcum[0]['cons'];
					$hldata[$t]['date']=$datesarray[$t];
	...
	function get_hardwares_device_data_energymeter_voltage_report_kondapur($data,$fromdate,$todate,$fromtime,$totime){
		$station_id=$data['station_id'];
		$hardware_name=$data['api_name'];
		$lineconnected=$data['LineConnected'];
		$utilityName=$data['UtilityName'];
		...
		foreach($meter_list as $meters){
			...
			if($meters['UtilityName']=="AFS project-C"){ 
				$resdata['PF'][$i]['meter']="Container office";
			}else if($meters['UtilityName']=="B4 Building"){
				$resdata['PF'][$i]['meter']="A4 Building";
			}else if($meters['UtilityName']=="Mains"){
				$resdata['PF'][$i]['meter']="Main I/C (EB)";
			}else if($meters['UtilityName']=="Fire Fighting"){
				$resdata['PF'][$i]['meter']="Fire pump panel";
			}else if($meters['UtilityName']=="LDB Pump"){
				$resdata['PF'][$i]['meter']="LDB (Pump room)";
			}else if($meters['UtilityName']=='PDB pump'){
				$resdata['PF'][$i]['meter']='PDB Panel';
			}else if($meters['UtilityName']=="LDB Street"){
				$resdata['PF'][$i]['meter']="LDB (Street light panel)";
			}else{
				$resdata['PF'][$i]['meter']=$meters['UtilityName'];
			}
			
	
			for($t=0;$t<count($datesarray);$t++){
				$check=$this->check_voltage_data('voltage',$meters['UtilityName'],$datesarray[$t]);
				if(count($check)==1){
					...
					// die();
					if($datesarray[$t]>=date('Y-m-d')){
						$queryvolt1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `LineConnected`='Voltage_1' AND `UtilityName`='".$meters['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
		// echo $queryvolt1;die();
						$v1 = $this->db->query($queryvolt1)->result();
						$voltage1_data=array_merge($voltage1_data,$v1);
	
						$queryvolt2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `LineConnected`='Voltage_2' AND `UtilityName`='".$meters['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
		
						$v2 = $this->db->query($queryvolt2)->result();
						$voltage2_data=array_merge($voltage2_data,$v2);
	
						$queryvolt3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `LineConnected`='Voltage_3' AND `UtilityName`='".$meters['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
	...
	function get_hardwares_device_data_energymeter_current_report_kondapur($data,$fromdate,$todate,$fromtime,$totime){
		$station_id=$data['station_id'];
		$hardware_name=$data['api_name'];
		$lineconnected=$data['LineConnected'];
		$utilityName=$data['UtilityName'];
		...
		foreach($meter_list as $meters){
			...
			for($t=0;$t<count($datesarray);$t++){
				$check=$this->check_current_data('current',$meters['UtilityName'],$datesarray[$t]);
				if(count($check)==1){
					...
					if($datesarray[$t]>=date('Y-m-d')){
						$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `LineConnected`='Current_1' AND `UtilityName`='".$meters['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
		// echo $querycurrent1;die();
						$c1 = $this->db->query($querycurrent1)->result();
						$current1_data=array_merge($current1_data,$c1);

						$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `LineConnected`='Current_2' AND `UtilityName`='".$meters['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
					
						$c2 = $this->db->query($querycurrent2)->result();
						$current2_data=array_merge($current2_data,$c2);

						$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `LineConnected`='Current_3' AND `UtilityName`='".$meters['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
	...
	function get_energymeter_list_kondapur($table_name){
		
		$this->db->select('');
        $this->db->from($table_name);     
        $this->db->where('LineConnected','KVA'); 
	...
	function get_hardwares_device_data($data){
		...
		$resdata['economy']=$finaleco;
		$resdata['availableFuel']=$dataStartEndFuel[0]->end;
		$resdata['filledper']=round(($dataStartEndFuel[0]->end/350)*100);
	...
	function getAHUData_rainbow($data){
		$date =date('Y-m-d');
		// $date ="2022-04-01";
		$result=array();
        ...
        select id, LineConnected,TxnDate, TxnTime, CurReading,Consumption ,LocationName,UomScale from protech_bms.hardware_station_consumption_data_rainbow_vikrampuri where
         TxnDate='".$date."' and LocationName=".$meter." and StationId='2022000093'  order by id desc 
    ...
    //echo $hvacData[0]->UtilityName;die();
    	for ($i=0; $i < count($hvacData); $i++) {
    		...
    		if($hvacData[$i]->LineConnected=='Return Air Temp'){
    			$result[$i1]['Return Air Temp']=$hvacData[$i]->CurReading."&deg;C";
    		}
    		if($hvacData[$i]->LineConnected=='Supply Air Temp'){
    			$result[$i1]['Supply Air Temp']=$hvacData[$i]->CurReading."&deg;C";
    		}
    		...
    		if($hvacData[$i]->LineConnected=='CHWS Temp'){
    			$result[$i1]['CW Sup Temp']=$hvacData[$i]->CurReading."&deg;C";
    		}
    		if($hvacData[$i]->LineConnected=='CHWR Temp'){
    			$result[$i1]['CH Ret Temp']=$hvacData[$i]->CurReading."&deg;C";
    		}
    		...
    		if($hvacData[$i]->LineConnected=='Unit Status F/B'){
    			$result[$i1]['Unit_Status F/B']=$hvacData[$i]->CurReading;
    		}
			if($hvacData[$i]->LineConnected=='Unit Status'){
    			$result[$i1]['Unit_Status']=$hvacData[$i]->CurReading;
    		}
			if($hvacData[$i]->LineConnected=='Cooling Status'){
    			$result[$i1]['Cooling_Status']=$hvacData[$i]->CurReading;
    		}
	...
	function getAHUData_rainbow_kondapur($data){
		$date =date('Y-m-d');
		$result=array();
  		for ($i1=0; $i1 < count($data); $i1++) { 
  			//$replaced = str_replace('_', ' ', $data[$i]['meter']);
        $meter = "'".$data[$i1]->MeterName."'";
        $query="select id, LineConnected,LocationName,UomScale,TxnDate, TxnTime, CurReading,Consumption from (
        select id, LineConnected,TxnDate, TxnTime, CurReading,Consumption ,LocationName,UomScale from protech_bms.hardware_station_consumption_data_rainbow_kondapur  where
         TxnDate='".$date."' and LocationName=".$meter." and StationId='2022000100'  order by id desc 
        ) as sub where id=sub.id group by sub.LineConnected";
    //    echo $query;die();
		$hvacData = $this->db->query($query)->result();
	...
	function getHavcList_rainbow(){
    	
        $query = "SELECT DISTINCT LocationName AS MeterName FROM hardware_station_consumption_data_rainbow_vikrampuri WHERE MeterName='HVAC' and MeterType=17";
    	$data = $this->db->query($query)->result();
    	return $data;
    }
	function getHavcList_rainbow_kondapur(){
    	
        $query = "SELECT DISTINCT LocationName AS MeterName FROM hardware_station_consumption_data_rainbow_kondapur WHERE MeterName='AC DX' and MeterType='DX01'";
    	$data = $this->db->query($query)->result();
    	return $data;
    }
	function getahuReportRainbow($data,$fromdate,$todate,$fromtime,$totime,$hardware,$stationid){
        $resultArray=array();
		for ($i1=0; $i1 < count($data); $i1++) { 
        ...
        $queryrat="SELECT TxnTime,CurReading FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Return Air Temp'";
    // echo $queryrat;die();
        $querysat="SELECT TxnTime,CurReading FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Supply Air Temp' ";    
        $queryrwt="SELECT TxnTime,CurReading FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='CHWR Temp'";
         $queryswt="SELECT TxnTime,CurReading FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='CHWS Temp' ";

         $queryactu="SELECT TxnTime,Consumption FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Actuator Level' ";
         $querystmp="SELECT TxnTime,Consumption FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Set Temp' ";
         $querypressure="SELECT TxnTime,Consumption FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Filter Pressure' ";
		 $querydeltaair="SELECT TxnTime,Consumption FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Delta T Air' ";
        ...
        { 
            ...
            $queryrr="SELECT SUM(Consumption) as Consumption FROM ".$hardware." where StationId=".$stationid." and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Unit RHT'    and TxnTime BETWEEN '".$from."' AND '".$to."'";
    ...
    function getahuReport($date,$meter){
        ...
        $queryrat="SELECT TxnTime,CurReading FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Return Air Temp'";
    //echo $queryrat;die();
        $querysat="SELECT TxnTime,CurReading FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Supply Air Temp' ";    
        $queryrwt="SELECT TxnTime,CurReading FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='CHWR Temp'";
         $queryswt="SELECT TxnTime,CurReading FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='CHWS Temp'";

         $queryactu="SELECT TxnTime,Consumption FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Actuator Level'";
         $querystmp="SELECT TxnTime,Consumption FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Set Temp' ";
         $querypressure="SELECT TxnTime,Consumption FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Filter Pressure'";
         $fulldata=array();
          for ($i=0; $i < 24; $i++) 
        { 
            if($i>9)
            ...
            $queryrr="SELECT SUM(Consumption) as Consumption FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and LineConnected='Unit RHT' and TxnTime >= '".$from."' AND TxnTime < '".$to."' ";
        ...
        // $queryruntime="SELECT Fromtime,Consumption FROM hardware_station_consumption_data_apollo where StationId=2022000093 and LocationName=".$meter." and TxnDate=".$date." and UtilityName='Unit Run Time' and FromTime NOT BETWEEN '23:58:00' AND '24:00:00' ";
	...
	function getRows($location,$date){
        $this->db->select('*');
        $this->db->from('rainbow_dashbord_data');        
		
		$this->db->where('location',$location);
		
		$this->db->where('data_date',$date);	  
        // $this->db->group_by('h.hardware_device');
        $res = $this->db->get()->result_array();  
		// echo $this->db->last_query();exit;
        return count($res);
	...
	public function insert($data = array()) {
        if(!empty($data)){
            ...
            // Insert member data
            $insert = $this->db->insert('rainbow_dashbord_data', $data);
	...
	function get_hardwares_device_list1_konda($device_id){
		$this->db->select('');
        $this->db->from('hardware as h');        
		$this->db->join('hardware_device as hd', 'hd.device_id=h.hardware_device','left');
		
		$this->db->where('h.client_id',29);
		if($device_id!=''){
			$this->db->where('h.hardware_device',$device_id);
		}	 
	...
	function getWaterMeterDashboardReport($fromdate,$todate){
		...
		// print_r($check_today);die();
			if(count($check_today)==1 && count($check_yesterday)==1){
				if($meters['LocationName']=='Fire Water Sump'){
					$capacity=462000;						
					$k=$k+round(($check_today[0]['water_level']*1.61)/1000);
					$cons=$cons+(round(($check_today[0]['water_level']*1.61)/1000)-round(($check_yesterday[0]['water_level']*1.61)/1000));
					$yest=$yest+round(($check_yesterday[0]['water_level']*1.61)/1000);						
					$fire_open+=round(($check_yesterday[0]['water_level']*1.61)/1000);
					$fire_close+=round(($check_today[0]['water_level']*1.61)/1000);
				}else if($meters['LocationName']=='Dom. Water Sump'){
					$capacity=840000;					
					$k=$k+round(($check_today[0]['water_level']*2.8)/1000);
					$cons=$cons+(round(($check_today[0]['water_level']*2.8)/1000)-round(($check_yesterday[0]['water_level']*2.8)/1000));
					$yest=$yest+round(($check_yesterday[0]['water_level']*2.8)/1000);						
					$domestic_open+=round(($check_yesterday[0]['water_level']*2.8)/1000);
					$domestic_close+=round(($check_today[0]['water_level']*2.8)/1000);
				}else{
					$capacity=143000;						
	...
	function rainbow_main_data($date){
		// echo $date;die();
		if($date!=''){
			...
			//echo $todayDate." ===".$yesterDay;die();
			$resultArray=array();
			$loc=array('vikrampuri','kondapur');
				//today
				for($i=0;$i<count($loc);$i++){
					$query1="SELECT * FROM rainbow_dashbord_data where data_date='".$yesterDay."' and location='".$loc[$i]."' ";
					$avgquery="SELECT round(SUM(water_consumption)/COUNT(*),2) as water_avg,round(SUM(`energy_consuption`)/COUNT(*),2) as energy_avg,round(SUM(`fuel_consumption`)/COUNT(*),2) as fuel_avg,round(SUM(`occupency`)/COUNT(*),2) as occupency_avg,round(SUM(`medical_gas`)/COUNT(*),2) as medical_gas_avg,round(SUM(`in_patient`)/COUNT(*),2) as in_patient_avg,round(SUM(`out_patient`)/COUNT(*),2) as out_patient_avg,COUNT(*) FROM `rainbow_dashbord_data` where `location`='".$loc[$i]."' ORDER BY `location` ASC";
				//echo $query1."<br>";
				$data1 = $this->db->query($query1)->result_array();
				$avgdata = $this->db->query($avgquery)->result_array();	
				
				
				if(count($data1)>0)	{				
					
					$resultArray[$i]['location']=$data1[0]['location'];
					$resultArray[$i]['date']=$yesterDay;
					$resultArray[$i]['water']=$data1[0]['water_consumption'];
					$resultArray[$i]['energy']=$data1[0]['energy_consuption'];
					$resultArray[$i]['fuel']=$data1[0]['fuel_consumption'];
					$resultArray[$i]['gas']=$data1[0]['medical_gas'];
					$resultArray[$i]['occupancy']=$data1[0]['occupency'];
					$resultArray[$i]['inpatient']=$data1[0]['in_patient'];
					$resultArray[$i]['outpatient']=$data1[0]['out_patient'];


					$resultArray[$i]['water_avg']=$avgdata[0]['water_avg'];
					$resultArray[$i]['energy_avg']=$avgdata[0]['energy_avg'];
					$resultArray[$i]['fuel_avg']=$avgdata[0]['fuel_avg'];
					$resultArray[$i]['gas_avg']=$avgdata[0]['medical_gas_avg'];
					$resultArray[$i]['occupancy_avg']=$avgdata[0]['occupency_avg'];
					$resultArray[$i]['inpatient_avg']=$avgdata[0]['in_patient_avg'];
					$resultArray[$i]['outpatient_avg']=$avgdata[0]['out_patient_avg'];

					if($avgdata[0]['water_avg']==0){
						$resultArray[$i]['water_avg_percent']=0;
					}else{
						$resultArray[$i]['water_avg_percent']=round((($data1[0]['water_consumption']-$avgdata[0]['water_avg'])*100)/$avgdata[0]['water_avg'],2);
					}
					if($avgdata[0]['energy_avg']==0){
						$resultArray[$i]['energy_avg_percent']=0;
					}else{
						$resultArray[$i]['energy_avg_percent']=round((($data1[0]['energy_consuption']-$avgdata[0]['energy_avg'])*100)/$avgdata[0]['energy_avg'],2);
					}
					
					if($avgdata[0]['fuel_avg']==0){
						
						$resultArray[$i]['fuel_avg_percent']=0;
					}else{
						
						$resultArray[$i]['fuel_avg_percent']=round((($data1[0]['fuel_consumption']-$avgdata[0]['fuel_avg'])*100)/$avgdata[0]['fuel_avg'],2);
					}
					if($avgdata[0]['gas_avg']==0){
						$resultArray[$i]['gas_avg_percent']=0;
					}else{
						$resultArray[$i]['gas_avg_percent']=round((($data1[0]['medical_gas']-$avgdata[0]['gas_avg'])*100)/$avgdata[0]['fuel_avg'],2);
					}
					if($avgdata[0]['occupency_avg']==0){
						$resultArray[$i]['occupancy_avg_percent']=0;
					}else{
						$resultArray[$i]['occupancy_avg_percent']=round((($data1[0]['occupency']-$avgdata[0]['occupency_avg'])*100)/$avgdata[0]['occupency_avg'],2);
					}	
					
					if($avgdata[0]['in_patient_avg']==0){
						$resultArray[$i]['inpatient_avg_percent']=0;
					}else{
						$resultArray[$i]['inpatient_avg_percent']=round((($data1[0]['in_patient']-$avgdata[0]['in_patient_avg'])*100)/$avgdata[0]['in_patient_avg'],2);
					}
					if($avgdata[0]['out_patient_avg']==0){
						$resultArray[$i]['outpatient_avg_percent']=0;
					}else{
						$resultArray[$i]['outpatient_avg_percent']=round((($data1[0]['out_patient']-$avgdata[0]['out_patient_avg'])*100)/$avgdata[0]['out_patient_avg'],2);
					}
					


					$resultArray[$i]['toatabeds']=$data1[0]['total_beds'];
					

				}else{
					$resultArray[$i]['location']=$loc[$i];
					$resultArray[$i]['date']=$yesterDay;
					$resultArray[$i]['water']="NA";
					$resultArray[$i]['energy']="NA";
					$resultArray[$i]['fuel']="NA";
					$resultArray[$i]['gas']="NA";
					$resultArray[$i]['occupancy']="NA";
					$resultArray[$i]['inpatient']="NA";
					$resultArray[$i]['outpatient']="NA";
					$resultArray[$i]['toatabeds']="NA";

					$resultArray[$i]['water_avg']="NA";
					$resultArray[$i]['energy_avg']="NA";
					$resultArray[$i]['fuel_avg']="NA";
					$resultArray[$i]['gas_avg']="NA";
					$resultArray[$i]['occupancy_avg']="NA";
					$resultArray[$i]['inpatient_avg']="NA";
					$resultArray[$i]['outpatient_avg']="NA";


					$resultArray[$i]['water_avg_percent']="NA";
					$resultArray[$i]['energy_avg_percent']="NA";
					
						$resultArray[$i]['gas_avg_percent']="NA";
						$resultArray[$i]['fuel_avg_percent']="NA";
					
					
					
					$resultArray[$i]['occupancy_avg_percent']="NA";
					$resultArray[$i]['inpatient_avg_percent']="NA";
					$resultArray[$i]['outpatient_avg_percent']="NA";
				}
				}
			...
			return $resultArray;
				...
				for($i=0;$i<count($loc);$i++){
					$query1="SELECT * FROM rainbow_dashbord_data where data_date='".$yesterDay."' and location='".$loc[$i]."' ";
					$avgquery="SELECT round(SUM(water_consumption)/COUNT(*),2) as water_avg,round(SUM(`energy_consuption`)/COUNT(*),2) as energy_avg,round(SUM(`fuel_consumption`)/COUNT(*),2) as fuel_avg,round(SUM(`occupency`)/COUNT(*),2) as occupency_avg,round(SUM(`in_patient`)/COUNT(*),2) as in_patient_avg,round(SUM(`out_patient`)/COUNT(*),2) as out_patient_avg,COUNT(*) FROM `rainbow_dashbord_data` where `location`='".$loc[$i]."' ORDER BY `location` ASC";
				//echo $query1."<br>";
				$data1 = $this->db->query($query1)->result_array();	
				$avgdata = $this->db->query($avgquery)->result_array();
