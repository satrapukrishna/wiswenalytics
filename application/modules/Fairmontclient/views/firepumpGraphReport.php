                        ...
                        <ul class="ScndLvl" id="reportsInnerId">
                            ...
                            <li>
                                ...
                                <ul class="ThrdLvl" id="eminnerid">                                   
                                    ...
                                    <li>
                                        <a href="<?php echo base_url(); ?>Fairmontclient/firepumpGraphReport" class="Lnk Actv act"><span class="Txt"> Graph Report</span></a>
                                    </li>
                                </ul>
                            </li>
                        </ul>
   ...
   // var totime = document.getElementById("totime").value;
    ...
    //for (var j = 0; j < noofmeters; j++) {
      $("#loading").css("display", "block");
       ...
       var urlString1 = "fairmontfirePumpRunningReport";
       var urlString2 = "getPressureData";
      $.ajax({
        url:urlString1,
        type : 'GET',
        async: true,
        ...
        success: function(data){
         ...
         // var economycontainer="container_economy"+kl;
          ...
          plotRunnGraph(data,runningContainer,kl);
...
function plotPressureGraph(data){
    ...
    ydata[i] = parseFloat(jsondata[i]['CurReading']);
    //myArray.push(sec);
  }
  Highcharts.chart('container_pipepressure', {
    chart: {
        type: 'line'
    },
    title: {
...
function plotRunnGraph(data,runnContainer,count){
...
var t=1;
    for (var j = 0; j < jsondata.length; j++) {
        for (var i = 0; i < jsondata[j].length; i++) {
            r=parseInt(jsondata[j][i]["RunningHours"]);
        dps1.push(r);
        runn.push(timeConvert(jsondata[j][i]["RunningHours"]));
        dps2.push(jsondata[j][i]["date"]);
        }
        
    }
 var meter= jsondata[0][0]['meter'];
 

  var cnt='container_runn1';
