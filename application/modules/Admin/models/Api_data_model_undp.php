...
class Api_data_model_undp extends CI_Model{
	...
	function get_energymeter_list_terotam($station_id){
		...
		$this->db->select('');
        $this->db->from('undp_devices_test'); 
	...
	function get_hardwares_device_data_energy_meters_tero($data){
		...
		for ($i=0; $i < count($power); $i++) {
			...
			//echo $enquery;die();kW
				...
				// echo $enquery;die();
				$resdata['uncf'][$i]['meter']=$power[$i]['UtilityName'];
	...
	function chech_energy_consumotion_terotam_monthly($meter,$ftdate,$tdate)
	{
		...
		$this->db->where('location_name',$meter);
		$this->db->where($where);
        $res = $this->db->get()->result_array();
		//echo $res[0]['report_date'];die();        
		//  echo "ll:".$this->db->last_query();die();       
	...
	FROM
		energy_consumption_report_tbl_undp
	WHERE
		report_date BETWEEN '".$ftdate."' AND '".$tdate."' AND location_name='".$meter."'";
	...
	function get_hardwares_device_data_energymeter_report_terotam_daynight_dashboard($data,$fromdate,$todate){
		...
		$date_from = strtotime('2025-06-15'); 
		...
		if($total==0){
			$resdata['tero']['meter']="ALL";
			$resdata['tero']['daycons']=0;
			$resdata['tero']['nightcons']=0;
			$resdata['tero']['totalcons']=0;
			$resdata['tero']['avgdaycons']=0;
			$resdata['tero']['avgnightcons']=0;
		}else{
			$resdata['tero']['meter']="ALL";
	...
	function get_hardwares_device_data_energymeter_report_terotam_week($data,$fromdate,$todate){
		...
		for ($i=0; $i < count($meter_list); $i++) {
			...
			for ($k=0; $k < count($datesarray); $k++)
				...
				$resdata['tero']['meter']=$meter_list[$i]['UtilityName'];
		...
		for ($i=0; $i < count($meter_list); $i++) {
			...
			for ($k=0; $k < count($datesarray); $k++)
				{ 
					if($sort==2){
						if($datesarray[$k]>=date('Y-m-d')){
							$resdata['tero'][$i][$k]['meter']=$meter_list[$i]['UtilityName'];
	...
	function get_hardwares_device_data_energymeter_day_report_hourly($data,$date){
		$station_id=$data['station_id'];
		$hardware_name=$data['api_name'];
		$lineconnected=$data['LineConnected'];
		...
		for ($i=0; $i < count($meter_list); $i++) {
			...
			else
			{
				...
				$queryconsutoday="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$date."' AND TxnTime BETWEEN '".$from."' AND '".$to."' AND LineConnected='kWh'	ORDER BY TxnTime";
				
				$datacontoday = $this->db->query($queryconsutoday)->result_array();
				if(is_null($datacontoday[0]['cons'])){
					array_push($cons, null);
				}else{
					array_push($cons, (float)round($datacontoday[0]['cons'],2));
				}
				
			
					
			}
			$resdata['unicef'][$i]['cons']=$cons;
			// $resdata['unicef'][$i]['time']=$time;
	...
	function get_hardwares_device_data_energymeter_day_report_terotam($data,$fromdate,$todate){
		...
		for ($i=0; $i < count($meter_list); $i++) {
			...
			for($t=0;$t<count($datesarray);$t++){
				...
				$c1 = $this->db->query($querycurrent1)->result_array();
				foreach ($c1 as $key => $value) {
					array_push($cons, (float)$value['CurReading']);
...
//
	...
	function get_hardwares_device_data_energymeter_monthly__year_to_year_report_terotam($data,$fromdate,$todate){
		...
		$lm1_fdate='2024-01-01';
		$lm1_tdate='2024-01-31';

		$lm2_fdate='2024-02-01';
		$lm2_tdate='2024-02-28';

		$lm3_fdate='2024-03-01';
		$lm3_tdate='2024-03-31';

		$lm4_fdate='2024-04-01';
		$lm4_tdate='2024-04-30';

		$lm5_fdate='2024-05-01';
		$lm5_tdate='2024-05-31';
		
		$lm6_fdate='2024-06-01';
		$lm6_tdate='2024-06-30';

		$lm7_fdate='2024-07-01';
		$lm7_tdate='2024-07-31';

		$lm8_fdate='2024-08-01';
		$lm8_tdate='2024-08-31';

		$lm9_fdate='2024-09-01';
		$lm9_tdate='2024-09-30';

		$lm10_fdate='2024-10-01';
		$lm10_tdate='2024-10-31';

		$lm11_fdate='2024-11-01';
		$lm11_tdate='2024-11-30';

		$lm12_fdate='2024-12-01';
		$lm12_tdate='2024-12-31';
		...
		for ($i=0; $i < count($meter_list); $i++) {
			...
			$lm1=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm1_fdate,$lm1_tdate);
			$lm2=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm2_fdate,$lm2_tdate);
			$lm3=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm3_fdate,$lm3_tdate);
			$lm4=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm4_fdate,$lm4_tdate);
			$lm5=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm5_fdate,$lm5_tdate);
			$lm6=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm6_fdate,$lm6_tdate);
			$lm7=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm7_fdate,$lm7_tdate);
			$lm8=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm8_fdate,$lm8_tdate);
			$lm9=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm9_fdate,$lm9_tdate);
			$lm10=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm10_fdate,$lm10_tdate);
			$lm11=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm11_fdate,$lm11_tdate);
			$lm12=$this->chech_energy_consumotion_terotam_monthly($meter_list[$i]['UtilityName'],$lm12_fdate,$lm12_tdate);
						...
						$data_last_year=[(float)$lm1,(float)$lm2,(float)$lm3,(float)$lm4,(float)$lm5,(float)$lm6,(float)$lm7,(float)$lm8,(float)$lm9,(float)$lm10,(float)$lm11,(float)$lm12];
				...
				$resdata['tero']['meter'][$i]=$meter_list[$i]['UtilityName'];
				$resdata['tero']['this_yaer_data'][$i]=$data_this_year;
				$resdata['tero']['this_yaer'][$i]="2025(".$meter_list[$i]['UtilityName'].")";
				$resdata['tero']['this_yaer_data_avg'][$i]=$data_this_year_avg;
				$resdata['tero']['last_yaer_data'][$i]=$data_last_year;
				$resdata['tero']['last_yaer'][$i]="2024(".$meter_list[$i]['UtilityName'].")";
	...
	function get_hardwares_device_data_energymeter_monthly_report_terotam($data,$fromdate,$todate){
		...
		for ($i=0; $i < count($meter_list); $i++) {
			...
			//$m3_sunday=$this->get_energy_consumption_weekday($meter_list[$i]['UtilityName'],$m3_fdate,$m3_tdate,1);
						...
						$data=[0,(float)$m6_sunday,(float)$m6_monday,(float)$m6_tusday,(float)$m6_wednesday,(float)$m6_thursday,(float)$m6_friday,(float)$m6_saturday,0,(float)$m7_sunday,(float)$m7_monday,(float)$m7_tusday,(float)$m7_wednesday,(float)$m7_thursday,(float)$m7_friday,(float)$m7_saturday];
