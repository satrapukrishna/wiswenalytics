...
class Demo_model extends CI_Model{
    ...
    function getSmsData($fromdate,$todate,$location){
             ...
             $alerts="SELECT count(*) as count FROM alerts_jntu WHERE  alert_date between  '".$fromdate."' and '".$todate."' and alert_name='".$location."'";
    ...
    function getUnacceptMaleConsolidate($fromdate,$todate,$stationid,$table_name,$location,$power){
                    ...
                    for ($k=0; $k < count($datesarray); $k++) { 
                        ...
                        $this->db->insert('jntu_odour_data', $odour_data);
                        } 
                    }
    ...
    function getFootfallConsolidate($fromdate,$todate,$stationid,$table_name,$location){
                    ...
                    for ($k=0; $k < count($datesarray); $k++) { 
                        ...
                        if(count($res_check)==1){
                            ...
                            $this->db->insert('jntu_footfall_data', $footfall_data);
                        }
    ...
    function getPowerConsolidate($fromdate,$todate,$stationid,$table_name,$location){
        ...
        for ($k=0; $k < count($datesarray); $k++) { 
            ...
            if(count($res_check)==1){
                ...
                $this->db->insert('jntu_power_data', $power_data);
...
function getWaterLeakConsolidate($fromdate,$todate,$stationid,$table_name,$location){
        ...
        $stationid = "'".$data['stationid']."'";
            ...
            for ($i=0; $i < 24; $i++) 
                { 
                    ...
                    $query="select SUM(TIME_TO_SEC(ToTime) - TIME_TO_SEC(FromTime))/60 AS `min` FROM $table where StationId=$stationid and TxnDate=".$date." and LineConnected='Odour Male' AND TxnTime >= '".$from."' AND TxnTime < '".$to."' ";
    ...
    function getOudorLevelMaleLiveDashboard($data){
        ...
        $table = $data['table'];
        $stationid = "'".$data['stationid']."'";
        $query_odour_male="SELECT TxnTime,Consumption as CurReading FROM $table where StationId=$stationid and LineConnected='Odour Male' and TxnDate=".$date." order by TxnTime ASC";
    ...
    function getOudorLevelFemaleLiveDashboard($data){
        ...
        $table = $data['table'];
        $stationid = "'".$data['stationid']."'";
        $query_odour_male="SELECT TxnTime,Consumption as CurReading FROM $table where StationId=$stationid and LineConnected='Odour Female' and TxnDate=".$date." order by TxnTime ASC";
        //echo $query_odour_male;die();
        $data_odour_male = $this->db->query($query_odour_male)->result();
            $xdata = array();
            $ydata = array();
            for ($or=0; $or < count($data_odour_male); $or++) { 
...
// echo $fromdate.$todate;die();
    ...
    function getFootfalAnalysisData($fromdate,$todate,$location,$branch,$stationid,$table_name,$table_name_live){
        ...
        $query_total="select round(SUM(Consumption)/2) as Consumption FROM $table_name where StationId=$stationid and TxnDate between '".$fromdate."' and '".$todate."' and LineConnected='Footfall Male'";
        $query_female_total="select round(SUM(Consumption)/2) as Consumption FROM $table_name where StationId=$stationid and TxnDate between '".$fromdate."' and '".$todate."' and LineConnected='Footfall Female'";
                       ...
                       // var_dump($data_total[0]['Consumption']);die();
                        ...
                        { 
                            ...
                            $query="select round(SUM(Consumption)/2) as Consumption FROM $table_name where StationId=$stationid and TxnDate between '".$fromdate."' and '".$todate."' and LineConnected='Footfall Male' AND TxnTime BETWEEN '".$from."' AND '".$to."'";
                            // echo $query;die();
                            

                            $query_female="select round(SUM(Consumption)/2) as Consumption FROM $table_name where StationId=$stationid and TxnDate between '".$fromdate."' and '".$todate."' and LineConnected='Footfall Female' AND TxnTime BETWEEN '".$from."' AND '".$to."' ";
