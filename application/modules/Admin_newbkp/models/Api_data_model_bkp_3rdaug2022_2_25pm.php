	...
	function get_hardwares_device_data_switch_control_report($data,$fromdate,$todate){
		...
		foreach($towers as $tower){
			...
			foreach($meter_list as $meters){
				...
				if($meters['LocationName']=='26th Floor'){
					$emergency="SELECT `CurReading`,TxnTime FROM $table_name WHERE LocationGroup='".$tower."' AND LocationName='19th Floor' AND TxnDate='".$datesarray[$t]."' AND UtilityName='Emergency' AND UomName='Status' AND CurReading=1 AND TxnTime <='12:00:00'   ORDER BY TxnTime desc LIMIT 1";
					$emergency_evng="SELECT `CurReading`,TxnTime FROM $table_name WHERE LocationGroup='".$tower."' AND LocationName='19th Floor' AND TxnDate='".$datesarray[$t]."' AND UtilityName='Emergency' AND UomName='Status' AND CurReading=1 AND TxnTime > '12:00:00'  ORDER BY TxnTime asc LIMIT 1";
				}else if($meters['LocationName']=='2nd Floor'){
