...
class Api_data_model extends CI_Model{
	...
	function getPressureToday($LineConnected){
      ...
      $querypressure="SELECT round(CurReading*0.076,2) as CurReading,FromTime,ToTime  FROM hardware_station_consumption_data where LineConnected='".$LineConnected."' AND TxnDate='".$todayDate."' and StationId='".$this->session->userdata('station_id')."' and FromTime NOT BETWEEN '23:56:00' AND '23:58:00' order by ToTime";
	...
	function getWaterLevel($api_name){
        ...
        select id, LocationName,TxnDate, FromTime, ToTime, Consumption ,UomScale from hardware_station_consumption_data where
         TxnDate=".$todayDate."  and StationId='".$this->session->userdata('station_id')."' and UtilityName='".$api_name."'  order by ToTime desc ) as sub where id=sub.id group by sub.LocationName limit 1";
        //echo $query;
