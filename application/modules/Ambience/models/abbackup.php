...
class Ambiance_model extends CI_Model{
    ...
    function getSodexoReport($data,$toiletId){
        ...
        //$date = "'2020-01-29'";
    	//echo $data['date'];die();
        $query1="SELECT round(SUM(Consumption)/2) as Consumption FROM app_device_station_consumtion_samsung where UtilityName='FootFall Male_Male RestRoom' and TxnDate=".$date."";

        $queryj1swp="SELECT SUM(Consumption) as Consumption FROM app_device_station_consumtion_samsung where UtilityName='Janitor_1_RestRoom' and TxnDate=".$date."";
        $queryj2swp="SELECT SUM(Consumption) as Consumption FROM app_device_station_consumtion_samsung where UtilityName='Janitor_2_RestRoom' and TxnDate=".$date."";

        $queryj1timings="SELECT FromTime,ToTime,Consumption FROM app_device_station_consumtion_samsung where UtilityName='Janitor_1_RestRoom' and TxnDate=".$date." and LocationWithUtility='Janitor_2_RestRoom' and Consumption>0";
        $queryj2timings="SELECT FromTime,ToTime,Consumption FROM app_device_station_consumtion_samsung where UtilityName='Janitor_2_RestRoom' and TxnDate=".$date." and LocationWithUtility='Janitor_2_RestRoom' and Consumption>0";
        
        $queryavg="SELECT SUM(Consumption) as Consumption FROM app_device_station_consumtion_samsung where UtilityName='FB Average_RestRoom' and TxnDate=".$date."";
        $querygood="SELECT SUM(Consumption) as Consumption FROM app_device_station_consumtion_samsung where UtilityName='FB Good_RestRoom' and TxnDate=".$date."";
        $querypoor="SELECT SUM(Consumption) as Consumption FROM app_device_station_consumtion_samsung where UtilityName='FB Poor_RestRoom' and TxnDate=".$date."";
        
        $queryj1="SELECT CurReading,FromTime,ToTime FROM app_device_station_consumtion_samsung where UtilityName='Janitor_1_RestRoom' and TxnDate=".$date." and Consumption>0";
        $queryj2="SELECT CurReading,FromTime,ToTime FROM app_device_station_consumtion_samsung where UtilityName='Janitor_1_RestRoom' and TxnDate=".$date." and Consumption>0";
        //echo $query1;die();
        ...
        $dataj1times = $this->db->query($queryj1timings)->result();
        $dataj2times = $this->db->query($queryj2timings)->result();
...
//janitor2
       ...
       // print_r($data1);die();
        ...
        $result[0]['swiped']=$dataj1swp[0]->Consumption+$dataj2swp[0]->Consumption;
        ...
        // if(count($dataj1swp)>0 || count($dataj1swp)>0){
             
        // }else{
        //    $result[0]['swiped']=0;  
        // }
        //  if(count($dataj2)>0){
        //     $result[0]['swiped']=$data[$i]->CurReading; 
        // }else{
        //    $result[0]['swiped']=0;  
        // }  
...
//$date = "'2019-12-09'";
    ...
    function getsodexoDataLive(){
        ...
        { 
            ...
            $query="select round(SUM(Consumption)/2) as Consumption FROM app_device_station_consumtion_samsung where StationId=2020000004 and TxnDate=".$date." and LocationWithUtility='FootFall Male_Male RestRoom' AND FromTime BETWEEN '".$from."' AND '".$to."' ";
            $data = $this->db->query($query)->result();
            //echo count($data);
            if(isset($data[0]->Consumption)){
        ...
        // echo "string";die();
               $date_from = strtotime($data['fromdate']); 
                ...
                for ($k=0; $k < count($datesarray); $k++) { 
                    ...
                    $query="select round(SUM(Consumption)/2) as Consumption FROM app_device_station_consumtion_samsung where StationId=2020000004 and TxnDate='".$datesarray[$k]."' and LocationWithUtility='FootFall Male_Male RestRoom' ";
                     $data = $this->db->query($query)->result();
                     if(isset($data[0]->Consumption)){
                 $footfall=$data[0]->Consumption;
                    }else{
...
function getsodexoDataCron(){
    ...
    $fulldata=array();
        ...
        { 
            ...
            $query="select Consumption FROM app_device_station_consumtion_samsung where StationId=2020000004 and TxnDate=".$date." and LocationWithUtility='FootFall Male_RestRoom' AND FromTime BETWEEN '".$from."' AND '".$to."' AND ToTime BETWEEN '".$from."' AND '".$to."'";
            $data = $this->db->query($query)->result();
            //echo $query;die();
            $m=0;
