...
<head>
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
	...
	<?php /*.l-99,.l-98,.l-97,.l-96,.l-95{
		height: 160px!important;
		background-color: rgba(0, 220, 95, 0.7);
	}
	.l-94,.l-93,.l-92,.l-91,.l-90{
		height: 150px!important;
		background-color: rgba(0, 220, 95, 0.7);
	}*/?>
...
input[type=range]:focus::-webkit-slider-runnable-track {
	...
	div.LiquidTank.Smll span.LiquidStatus{    right: -97px!important;font: 600 24px 'Open Sans'!important;}
    ...
    <div id="MnCtnr" class="DshMnCtnr">
		...
		<!-- water level code starts -->
            <div class="DshBrdSctn" style="padding: 10px 30px 10px 38px;">
				...
				<?php foreach($water_terrace as $rec){
				...
				<div>
					<div class="fire_waterpump">
						...
						<?php 
							$current_level=$rec[0]->Consumption*$rec[0]->Multiplier%1000;
							$total_level=$rec['capacity'];
							// $avial_level=$water_level_sen->Consumption;
							$level=round(($current_level/$total_level)*100);
							
							if($level==100){?>
								<div class="Liquid Liquidhigh l-100"></div>
							<?php }else if($level>=50){?>
							...
							<?php }else{
								?>
								<div class="Liquid Liquidhigh"></div>
								<?php
							}	?>
						</div>
						</div>
						...
						<span class="SctnTtl tank-title"><?php echo $rec[0]->LocationName?></span>
						</div>
					   
						<ul class="SctnDtlsGrdTbl">
						<li><div class="ClLft">Total Capacity</div><div class="ClRgt"><?php echo $rec['capacity']?> KL</div></li>
						<li><div class="ClLft">Current Level</div><div class="ClRgt"><?php echo $current_level?> KL</div></li>
				...
				<div class="bxslider555" id="bxid">
						...
						<?php foreach($water_basement as $rec1){
				...
				<div>
					<div class="fire_waterpump">
						...
						<div class="LiquidTank">
						<?php 
							$current_level=$rec1[0]->Consumption*$rec1[0]->Multiplier%1000;
							$total_level=$rec1['capacity'];
							// $avial_level=$water_level_sen->Consumption;
							$level=round(($current_level/$total_level)*100);
						...
						<span class="SctnTtl tank-title"><?php echo $rec1[0]->LocationName?></span>
						</div>
					   
						<ul class="SctnDtlsGrdTbl">
						<li><div class="ClLft">Total Capacity</div><div class="ClRgt"><?php echo $rec1['capacity']?> KL</div></li>
						<li><div class="ClLft">Current Level</div><div class="ClRgt"><?php echo $current_level?> KL</div></li>
						<li><div class="ClLft">Filled</div><div class="ClRgt"><?php echo $level?>%</div></li>
						</ul>
						</div>
			...
			<?php if(modules::run('Admin/Site/authlink','water_firepump')){ 
					...
					$firepumps=$this->Api_data_model->get_firepumpdata(20,'Pumps');
					$pumps=$firepumps;	
					
					$fireData=$this->Api_data_model->getDashBoardData($firepumps->result());
					$water_level=$this->Api_data_model->get_firepumpdata(20,'Water Level Sensors');
					// echo "<pre>";print_r($water_level->result());exit;
					$line_pressure=$this->Api_data_model->get_firepumpdata(20,'Pressure Sensor');
					...
					foreach($line_pressure->result() as $rec){
						...
						for($i=0;count($data1[$j])>$i;$i++){
							...
							$pdata[$rec->LineConnected]['timings'][$i]=$data1[$j][$i]->TxnTime;				
						}			
					$j++;
					}
                ...
                <div class="DshBrdSctnDtls WhtBkgrnd FrPmp device devicebox1111" id="devicebox1111">
                       ...
                       <?php
                        ...
                        <tr>
                            ...
                            <td>
								...
								<?php }else{ ?>
								<?php /*<span class="Txt Stts Manual" >NA</span>*/?>
								<span class="Txt Stts auto" id=""></span>
							...
							<?php }else{
								...
								<td>
                                <span class="Txt MblTtl">Today</span>
                                <span class="Txt"><?php echo ($fireData[$i]['TodayRunn']!="NA"?$fireData[$i]['TodayRunn'].' Min':'0 Min');?></span>
                            </td>
                            <td>
                                <span class="Txt MblTtl">Yesterday</span>
                                <span class="Txt"><?php echo ($fireData[$i]['YesterdayRunn']!="NA"?$fireData[$i]['YesterdayRunn'].'0 Min':'0 Min');?></span>
                            </td>
                            <td>
                                <span class="Txt MblTtl">Last Week</span>
                                <span class="Txt"><?php echo ($fireData[$i]['LastWeekRunn']!="NA"?$fireData[$i]['LastWeekRunn'].'0 Min':'0 Min');?></span>
                            </td>
								<?php 
							}?>
                        </tr>
					...
					<div class="fire_sub">
						<div class="fire_dgset">
							...
							<ul class="SctnDtlsGrdTbl">
							<li><div class="ClLft">Fuel Consumption</div><div class="ClRgt"><?php echo $dgdata[0]['fuelConsumed']?> Ltrs</div></li>
							<li><div class="ClLft">Running Time</div><div class="ClRgt"><?php echo $dgdata[0]['runningHours']?></div></li>
							<li><div class="ClLft">Fuel Added</div><div class="ClRgt"><?php echo $dgdata[0]['fuelAdded']?> Ltrs</div></li>
							<li><div class="ClLft">Available Fuel</div><div class="ClRgt"><?php echo $dgdata[0]['currentFuel']?> Ltrs</div></li>
							<li><div class="ClLft">Fuel Removed</div><div class="ClRgt"><?php echo $dgdata[0]['fuelTheft']?> Ltrs</div></li>
							 <li><div class="ClLft">Battery Voltage</div><div class="ClRgt"><?php echo $dgdata[0]['batVoltage']+23?> Volts</div></li>
							</ul>
							</div>
						...
						foreach($water_level->result_array() as $rec){
							...
							// echo "<pre>";print_r($water_level_sen);exit;
						?>
						...
						$total_level=$rec['Volume'];
							//$total_level=200;
							$avial_level=round($water_level_sen->Consumption);
							$level=round(($avial_level/$total_level)*100);
						if($level==100){?>
								<div class="Liquid Liquidhigh l-100"></div>
							<?php }else if($level>=50){?>
								<div class="Liquid Liquidhigh l-70"></div>
						...
						<ul class="SctnDtlsGrdTbl">
						<li><div class="ClLft">Total Capacity</div><div class="ClRgt"><?php echo $total_level?>KL</div></li>
						<li><div class="ClLft">Current Level</div><div class="ClRgt"><?php echo $avial_level?>KL</div></li>
						<li><div class="ClLft">Filled</div><div class="ClRgt"><?php echo $level?>%</div></li>
						</ul>
						</div>
						</div>
						...
						<table class="SctnDtlsDualGrd">
							...
							foreach($pumps->result_array() as $row){
								...
								<tr>
									...
									<td class="Col-5">
										<?php if($row['CutOutPressure']!='Manual'){ ?>
                ...
                <span class="status-off">OFF</span>
					...
					<ul class="SctnDtlsGrdTbl lpgdiv">
                     ...
                     <div class=" EnrgyMtrGuge" style="padding:0px;width:33%;float:left;margin-top:20px">
                        ...
                        <div id="container_speed_11" style="height:230px;border: 1px solid #ddd;border-radius: 10px;margin:10px">
					...
					<div id="container_pressure_11" style="height:230px;border: 1px solid #ddd;border-radius: 10px;">
...
var pressurecontainer2="container_pressure_11";
var speedcontainer2="container_speed_11";
