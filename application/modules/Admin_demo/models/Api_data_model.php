...
class Api_data_model extends CI_Model{
	...
	function get_hardwares_device_data_borewell($data){
		...
		$abs_diff = $later->diff($earlier)->format("%a")+1; //3
			...
			$check_t=$this->chech_borewell_running($lineconnected,$utilityName,$todayDate,$LocationName);
			$today_runn=(int)$check_t[0]['running_min2'];
			...
			$resdata['todayconsumption']=$this->secondsToTime(round($today_runn*60));
			;
	...
	function get_hardwares_device_data_flowmeter($data){
		...
		$table_name=$this->get_table_name($station_id);
		$todayDate="2022-01-30";
		$yesterDay = "2022-01-29";
		...
		$today_check=$this->chech_water_consumption($lineconnected,$utilityName,$todayDate,$LocationName);
		$todayday_consumption=(float)$today_check[0]['consumption'];
	...
	function get_hardwares_device_data_energy_meters($data){
		...
		foreach($meter_list as $meters){
				...
				$today_check=$this->chech_energy_consumotion($meters['LocationName'],$todayDate);
			    $today_cons=(float)$today_check[0]['consumption'];
		...
		if($meterserial=='71'){
			...
			foreach($firepumpList as $list){
				if($list['fire_pump_name']=='Panel Power Supply'){
					...
					$tod_check=$this->check_firepump_running('J_Pump Auto RHT','Old Fire Pump',$todayDate,$list['fire_pump_name'],'0071');
					$today_runn=(int)$tod_check[0]['running_min2'];
					...
					$tod_check=$this->check_firepump_running('Auto Mode RHT','Old Fire Pump',$todayDate,$list['fire_pump_name'],'0071');
					$today_runn=(int)$tod_check[0]['running_min2'];
					...
					$tod_check=$this->check_firepump_running('DG On RHT','Old Fire Pump',$todayDate,$list['fire_pump_name'],'0071');
					$today_runn=(int)$tod_check[0]['run'];
	...
	function get_hardwares_device_data_hydro($data){
		...
		$resultArray=array();
			...
			foreach($hydrolist as $list){
				...
				$today_check=$this->chech_hydro_running($list['UtilityName'],$todayDate,'0061');
				$today_runn=(float)$today_check[0]['running_min2']*60;
