				...
				if($list['fire_pump_name']=='Jockey Pump'){
					...
					for ($k=0; $k < count($run_status_data); $k++) { 
						if($run_status_data[$k]['Consumption']>0){
							$message=$hardware_name.':: '.$list['fire_pump_name'].' is ON on '.$todayDate." ".$run_status_data[$k]['TxnTime'];
							$msg=array(
								'client_id'=>$client_id,
								'hardware_device'=>$hardware_device,
								'hardware_id'=>$hardware_id,
...
//echo $add;die();
	...
	function sendMail_aaurush($msg,$alert_name,$date){
	   ...
	   'smtp_port' => 465,
	   'smtp_user' => 'wenalytics@gmail.com',
	   'smtp_pass' => 'wenalytics@123',
	   'mailtype'  => 'html', 
	   'charset' => 'utf-8',
	   'wordwrap' => TRUE
   ...
   $list = array('krishna@wenalytics.com','sunilmanohar@wenalytics.com','Madhan.RajR@ap.jll.com','divyani.devaliya@firstspacedevelopment.in','dinesh.malokar@firstspacedevelopment.in','sivaprathab.r@firstspacedevelopment.in','hriday@wenalytics.com');
   $this->email->to($list);
   ...
   $this->email->send();
	...
	function check_notification($data){
		$query="SELECT * FROM alerts where client_id='".$data['client_id']."' and hardware_device='".$data['hardware_device']."' and hardware_id='".$data['hardware_id']."' and alert_date='".$data['alert_date']."' and alert_time='".$data['alert_time']."' and alert_name='".$data['alert_name']."' and alert_read='".$data['alert_read']."' ";
            // echo $query;die()
