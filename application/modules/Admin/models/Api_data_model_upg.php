	...
	function get_hardwares_device_data_waterlevelmeter($data)
{
    ...
    if (!empty($table_name)) {
        $this->db->select("ROUND((CurReading * {$multiplier}) / 1000, 2) AS CurReading", false);
        $this->db->from($table_name);
        $this->db->where('StationId', $station_id);
        $this->db->where('UtilityName', $utilityName);
        $this->db->where('LocationName', $locationName);
        $this->db->order_by('TxnDate', 'DESC');
        $this->db->order_by('TxnTime', 'DESC');
        $this->db->limit(1);

        $datawaterlevel = $this->db->get()->row_array();

        $waterlevel = !empty($datawaterlevel['CurReading']) ? (float)$datawaterlevel['CurReading'] : 0;

        $resdata['currentlevel']   = $waterlevel;
        $resdata['filledpercent']  = ($resdata['capacity'] > 0) ? round(($waterlevel / $resdata['capacity']) * 100, 2) : 0;
        $resdata['filledpercent_1'] = ($resdata['capacity'] > 0) ? round(($waterlevel / $resdata['capacity']) * 100) : 0;
    } else {
...
function get_hardwares_device_data_borewell($data)
{
    ...
    $tableName  = $this->get_table_name($stationId);

    $result = array(
        'meter'                => $dashboardName,
        'todayconsumption'     => '0 hours 0 minutes',
        'yesterdayconsumption' => '0 hours 0 minutes',
        'monthly_consumption'  => '0 hours 0 minutes',
        'status'               => 0
    );
    ...
    $dateTo     = strtotime($todayDate);

    for ($time = $dateFrom; $time <= $dateTo; $time += 86400) {
        $monthDates[] = date('Y-m-d', $time);
    }

    $daysCount = count($monthDates);

    $todayRunningSecs = $this->get_borewell_running_seconds($tableName, $utilityName, $todayDate, $lineConnected, $locationName);
    $todayRunning     = round($todayRunningSecs - ($todayRunningSecs * 0.15));

    $yesterdayCheck = $this->chech_borewell_running($lineConnected, $utilityName, $yesterDay, $locationName);
    if (!empty($yesterdayCheck)) {
        $yesterdayRunning = (int) $yesterdayCheck[0]['running_min2'];
    } else {
        $yesterdaySecs    = $this->get_borewell_running_seconds($tableName, $utilityName, $yesterDay, $lineConnected, $locationName);
        $yesterdayRunning = round($yesterdaySecs - ($yesterdaySecs * 0.15));
    }

    $monthlyRunning = 0;
    foreach ($monthDates as $reportDate) {
        $monthCheck = $this->chech_borewell_running($lineConnected, $utilityName, $reportDate, $locationName);

        if (!empty($monthCheck)) {
            $monthlyRunning += (int) $monthCheck[0]['running_min2'];
        } else {
            $monthSecs = $this->get_borewell_running_seconds($tableName, $utilityName, $reportDate, $lineConnected, $locationName);
            $monthlyRunning += round($monthSecs - ($monthSecs * 0.15));
        }
    }

    $monthlyRunningWithToday = $monthlyRunning + $todayRunning;

    $result['todayconsumption']     = $this->secondsToTime($todayRunning);
    $result['yesterdayconsumption'] = $this->secondsToTime($yesterdayRunning * 60);
    $result['monthly_consumption']  = $this->secondsToTime(round(($monthlyRunningWithToday * 60) / max($daysCount, 1)));

    return $result;
}

function get_borewell_running_seconds($tableName, $utilityName, $reportDate, $lineConnected, $locationName)
{
    $row = $this->db
        ->select('SUM(TIME_TO_SEC(ToTime) - TIME_TO_SEC(FromTime)) AS secs', false)
        ->from($tableName)
        ->where('UtilityName', $utilityName)
        ->where('TxnDate', $reportDate)
        ->where('LineConnected', $lineConnected)
        ->where('LocationName', $locationName)
        ->where('Consumption >', 0)
        ->where('(TIME_TO_SEC(ToTime) - TIME_TO_SEC(FromTime)) > 0', null, false)
        ->get()
        ->row_array();

    return !empty($row['secs']) ? (int) $row['secs'] : 0;
}
	...
	function get_hardwares_device_data_flowmeter($data){
		...
		if(isset($table_name)){
			...
			if($LocationName=='Tpi' || $LocationName=='A4 Building'){
				$queryconsutoday="SELECT SUM(Consumption)*10 as cons FROM $table_name_live WHERE UtilityName='".$utilityName."' AND TxnDate='".$todayDate."' AND LineConnected='".$lineconnected."' AND LocationName='".$LocationName."'";
			}else{
				$queryconsutoday="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE UtilityName='".$utilityName."' AND TxnDate='".$todayDate."' AND LineConnected='".$lineconnected."' AND LocationName='".$LocationName."'";
			}
			...
			for ($k=0; $k < count($datesarray); $k++)
				{ 
					...
					if(count($check)==1){
						...
						if($datesarray[$k]>=date('Y-m-d')){
							$queryc="SELECT round((SUM(Consumption)/1000)) as cons FROM $table_name_live WHERE UtilityName='".$utilityName."' AND TxnDate='".$datesarray[$k]."' AND LineConnected='".$lineconnected."' AND LocationName='".$LocationName."'";
						}else{
				...
				if(count($yest_check)==1){
					...
					if($LocationName=='Tpi' || $LocationName=='A4 Building'){
						$queryconsuyest="SELECT SUM(Consumption)*10 as cons FROM $table_name_live WHERE UtilityName='".$utilityName."' AND TxnDate='".$yesterDay."' AND LineConnected='".$lineconnected."' AND LocationName='".$LocationName."'";
					}else{
	...
	function get_hardwares_device_data_energymeter_current_report_undp($data,$fromdate,$todate){
		...
		$meter_list_undp=$this->get_energymeter_list_cv_undp(2023000304);
		$meter_list_uncw=$this->get_energymeter_list_cv_undp(2023000303);
		$meter_list_unew=$this->get_energymeter_list_cv_undp(2023000300);
		$meter_list_unff=$this->get_energymeter_list_cv_undp(2023000302);
		$meter_list_unww=$this->get_energymeter_list_cv_undp(2023000301);
		$meter_list_unsg=$this->get_energymeter_list_cv_undp(2024000143);
		$meter_list_unab=$this->get_energymeter_list_cv_undp(2024000144);
		...
		for ($i=0; $i < count($meter_list_uncw); $i++) {
			...
			for($t=0;$t<count($datesarray);$t++){
					...
					if($datesarray[$t]>=date('Y-m-d')){
							...
							$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_1' AND  `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
		
							$c1 = $this->db->query($querycurrent1)->result();
							$current1_data=array_merge($current1_data,$c1);
	
							$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_2' AND `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
						
							$c2 = $this->db->query($querycurrent2)->result();
							$current2_data=array_merge($current2_data,$c2);
	
							$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_3' AND `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			...
			for ($i=0; $i < count($meter_list_unew); $i++) {
				...
				for($t=0;$t<count($datesarray);$t++){
						...
						if($datesarray[$t]>=date('Y-m-d')){
								...
								$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_1' AND  `UtilityName`='".$meter_list_unew[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			
								$c1 = $this->db->query($querycurrent1)->result();
								$current1_data=array_merge($current1_data,$c1);
		
								$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_2' AND `UtilityName`='".$meter_list_unew[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
							
								$c2 = $this->db->query($querycurrent2)->result();
								$current2_data=array_merge($current2_data,$c2);
		
								$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_3' AND `UtilityName`='".$meter_list_unew[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			...
			for ($i=0; $i < count($meter_list_unww); $i++) {
				...
				for($t=0;$t<count($datesarray);$t++){
						...
						if($datesarray[$t]>=date('Y-m-d')){
								...
								$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_1' AND  `UtilityName`='".$meter_list_unww[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			
								$c1 = $this->db->query($querycurrent1)->result();
								$current1_data=array_merge($current1_data,$c1);
		
								$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_2' AND `UtilityName`='".$meter_list_unww[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
							
								$c2 = $this->db->query($querycurrent2)->result();
								$current2_data=array_merge($current2_data,$c2);
		
								$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_3' AND `UtilityName`='".$meter_list_unww[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
...
for ($i=0; $i < count($meter_list_unsg); $i++) {
	...
	for($t=0;$t<count($datesarray);$t++){
			...
			if($datesarray[$t]>=date('Y-m-d')){
					...
					$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_1' AND  `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";

					$c1 = $this->db->query($querycurrent1)->result();
					$current1_data=array_merge($current1_data,$c1);

					$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_2' AND `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
				
					$c2 = $this->db->query($querycurrent2)->result();
					$current2_data=array_merge($current2_data,$c2);

					$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Current_3' AND `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
		...
		for ($i=0; $i < count($meter_list_uncw); $i++) {
			...
			for($t=0;$t<count($datesarray);$t++){
					...
					if($datesarray[$t]>=date('Y-m-d')){
							...
							$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_1' AND  `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
		
							$c1 = $this->db->query($querycurrent1)->result();
							$voltage1_data=array_merge($voltage1_data,$c1);
	
							$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_2' AND `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
						
							$c2 = $this->db->query($querycurrent2)->result();
							$voltage2_data=array_merge($voltage2_data,$c2);
	
							$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_3' AND `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			...
			for ($i=0; $i < count($meter_list_unew); $i++) {
				...
				for($t=0;$t<count($datesarray);$t++){
						...
						if($datesarray[$t]>=date('Y-m-d')){
								...
								$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_1' AND  `UtilityName`='".$meter_list_unew[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			
								$c1 = $this->db->query($querycurrent1)->result();
								$voltage1_data=array_merge($voltage1_data,$c1);
		
								$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_2' AND `UtilityName`='".$meter_list_unew[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
							
								$c2 = $this->db->query($querycurrent2)->result();
								$voltage2_data=array_merge($voltage2_data,$c2);
		
								$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_3' AND `UtilityName`='".$meter_list_unew[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			...
			for ($i=0; $i < count($meter_list_unww); $i++) {
				...
				for($t=0;$t<count($datesarray);$t++){
						...
						if($datesarray[$t]>=date('Y-m-d')){
								...
								$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_1' AND  `UtilityName`='".$meter_list_unww[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
			
								$c1 = $this->db->query($querycurrent1)->result();
								$voltage1_data=array_merge($voltage1_data,$c1);
		
								$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_2' AND `UtilityName`='".$meter_list_unww[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
							
								$c2 = $this->db->query($querycurrent2)->result();
								$voltage2_data=array_merge($voltage2_data,$c2);
		
								$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_3' AND `UtilityName`='".$meter_list_unww[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
...
for ($i=0; $i < count($meter_list_unsg); $i++) {
	...
	for($t=0;$t<count($datesarray);$t++){
			...
			if($datesarray[$t]>=date('Y-m-d')){
					...
					$querycurrent1="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_1' AND  `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";

					$c1 = $this->db->query($querycurrent1)->result();
					$voltage1_data=array_merge($voltage1_data,$c1);

					$querycurrent2="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_2' AND `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
				
					$c2 = $this->db->query($querycurrent2)->result();
					$voltage2_data=array_merge($voltage2_data,$c2);

					$querycurrent3="SELECT round(CurReading,2) as CurReading ,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."'  AND `LineConnected`='Voltage_3' AND `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."'  ORDER BY TxnDate ASC,TxnTime ASC";
...
for ($i=0; $i < count($Other_Energy_End_uses); $i++) {
	...
	function get_energy_consumotion_undp($location_name,$fdate,$tdate)
	{
		$f="'$fdate'";
		$t="'$tdate'";
		...
		$this->db->where('report_date BETWEEN ' . $f . ' AND ' . $t);
	...
	function get_energymeter_lists_station($st1,$st2){

		$qry="SELECT * FROM `undp_devices` WHERE `LineConnected` = 'kWh' AND `StationId` IN (2024000143,2024000144) GROUP BY `UtilityName`";
	...
	function get_hardwares_device_data_evch_report_undp($data,$year){
		$meter_list_unsg=$this->get_energymeter_lists_station(2024000143,2024000144);
		$k=0;
		for ($i1=1; $i1 < 13; $i1++) { 
			$number = cal_days_in_month(CAL_GREGORIAN, $i1, $year);
			$fromdate=$year."-".$i1."-01";
			$todate=$year."-".$i1."-".$number;		
			
			for ($i=0; $i < count($meter_list_unsg); $i++) {
			
						$check=$this->get_energy_consumotion_undp($meter_list_unsg[$i]['UtilityName'],$fromdate,$todate);
						
							$monthNum = $i1;
 							$monthName = date("F", mktime(0, 0, 0, $monthNum, 10));
								$resdata['data'][$k][$i]['meter']=$meter_list_unsg[$i]['UtilityName'];
								$resdata['data'][$k][$i]['month']=$monthName;
						   
								$resdata['data'][$k][$i]['consumption']=round($check[0]['cons'],2);
				
								$resdata['data'][$k][$i]['ratepercost']=8.5;
								$resdata['data'][$k][$i]['percost']=round($resdata['data'][$k][$i]['consumption']*8.5,2);
		...
		return $resdata;
		// echo json_encode($resdata['data'][0]);die();
	...
	function get_hardwares_device_data_energymeter_report_undp($data,$fromdate,$todate,$sort){
		...
		$meter_list_undp=$this->get_energymeter_list_undp(2023000304);
		$meter_list_uncw=$this->get_energymeter_list_undp(2023000303);
		$meter_list_unew=$this->get_energymeter_list_undp(2023000300);
		$meter_list_unff=$this->get_energymeter_list_undp(2023000302);
		$meter_list_unww=$this->get_energymeter_list_undp(2023000301);
		$meter_list_unsg=$this->get_energymeter_list_undp(2024000143);
		$meter_list_unab=$this->get_energymeter_list_undp(2024000144);
		...
		// echo json_encode($resdata['unsg']);die();
		//return $resdata;		
		$fullData=array();	
	//if($sort==2){
	$rs=[];
...
for ($i=0; $i < count($meter_list_unww); $i++) {
	...
	function get_energymeter_list_undp111($station_id){
	...
	function get_hardwares_device_data_energymeter_report_undp1($table_name,$meter,$date){
		...
		for ($i1=0; $i1 < 24; $i1++) 
			...
			else
			{
				...
				$queryconsutoday="SELECT SUM(Consumption) as cons FROM $table_name WHERE `UtilityName`='".$meter."' AND `TxnDate`='".$date."' AND TxnTime BETWEEN '".$from."' AND '".$to."' AND LineConnected='kWh'	ORDER BY TxnTime";
				
				
				$datacontoday = $this->db->query($queryconsutoday)->result();
				$resdata1[$i1]['consumption']=$datacontoday[0]->cons;
	...
	function get_hardwares_device_data_energy_meters_undp($data){
		...
		for ($i=0; $i < count($meter_list_undp); $i++) {
				...
				$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_undp[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
				 $kwdata = $this->db->query($enquery_kw)->result_array();
				 $resdata['undp'][$i]['live']=$this->get_hardwares_device_data_energymeter_report_undp1($table_name_live,$meter_list_undp[$i]['UtilityName'],$todayDate);;
				if($meter_list_undp[$i]['ModelNo']=='EM6400NG' || $meter_list_undp[$i]['ModelNo']=='EM6436H' || $meter_list_undp[$i]['ModelNo']=='LG5310'){	
			...
			//echo $enquery;die();kW
				...
				$resdata['undp'][$i]['model']=$meter_list_undp[$i]['ModelNo'];
				}else{
					$resdata['undp'][$i]['pf']="NA";
					$resdata['undp'][$i]['model']=$meter_list_undp[$i]['ModelNo'];
					$resdata['undp'][$i]['current1']="NA";
		...
		for ($i=0; $i < count($meter_list_uncw); $i++) {
			$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
				...
				$resdata['uncw'][$i]['live']=$this->get_hardwares_device_data_energymeter_report_undp1($table_name_live,$meter_list_uncw[$i]['UtilityName'],$todayDate);;
				...
				if($meter_list_uncw[$i]['ModelNo']=='EM6400NG' || $meter_list_uncw[$i]['ModelNo']=='EM6436H' || $meter_list_uncw[$i]['ModelNo']=='LG5310'){	
		
					
					$enquery_current1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
					// echo $enquery_current1;die();
				 $c1_data = $this->db->query($enquery_current1)->result_array();
		
				 $enquery_current2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
				 $c2_data = $this->db->query($enquery_current2)->result_array();
		
				 $enquery_current3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
				 $c3_data = $this->db->query($enquery_current3)->result_array();
		
		
				 $enquery_volt1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
				 $v1_data = $this->db->query($enquery_volt1)->result_array();
		
				 $enquery_volt2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
				 $v2_data = $this->db->query($enquery_volt2)->result_array();
		
				 $enquery_volt3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_uncw[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
				 $v3_data = $this->db->query($enquery_volt3)->result_array();
			...
			//echo $enquery;die();kW
				...
				$resdata['uncw'][$i]['model']=$meter_list_uncw[$i]['ModelNo'];
		...
		for ($i=0; $i < count($meter_list_unew); $i++) {
			$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
				...
				$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
				 $kwdata = $this->db->query($enquery_kw)->result_array();
				 $resdata['unew'][$i]['live']=$this->get_hardwares_device_data_energymeter_report_undp1($table_name_live,$meter_list_unew[$i]['UtilityName'],$todayDate);;
				if($meter_list_unew[$i]['ModelNo']=='EM6400NG' || $meter_list_unew[$i]['ModelNo']=='EM6436H' || $meter_list_unew[$i]['ModelNo']=='LG5310'){	
		
					
					$enquery_current1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
					// echo $enquery_current1;die();
				 $c1_data = $this->db->query($enquery_current1)->result_array();
		
				 $enquery_current2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
				 $c2_data = $this->db->query($enquery_current2)->result_array();
		
				 $enquery_current3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
				 $c3_data = $this->db->query($enquery_current3)->result_array();
		
		
				 $enquery_volt1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
				 $v1_data = $this->db->query($enquery_volt1)->result_array();
		
				 $enquery_volt2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
				 $v2_data = $this->db->query($enquery_volt2)->result_array();
		
				 $enquery_volt3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unew[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
				 $v3_data = $this->db->query($enquery_volt3)->result_array();
			...
			//echo $enquery;die();kW
				...
				$resdata['unew'][$i]['model']=$meter_list_unew[$i]['ModelNo'];
				}else{
					$resdata['unew'][$i]['pf']="NA";
					$resdata['unew'][$i]['model']=$meter_list_unew[$i]['ModelNo'];
					$resdata['unew'][$i]['current1']="NA";
		...
		for ($i=0; $i < count($meter_list_unff); $i++) {
			$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unff[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
				...
				$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unff[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
				 $kwdata = $this->db->query($enquery_kw)->result_array();
				 $resdata['unff'][$i]['live']=$this->get_hardwares_device_data_energymeter_report_undp1($table_name_live,$meter_list_unff[$i]['UtilityName'],$todayDate);;
				if($meter_list_unff[$i]['ModelNo']=='EM6400NG' || $meter_list_unff[$i]['ModelNo']=='EM6436H' || $meter_list_unff[$i]['ModelNo']=='LG5310'){	
			...
			//echo $enquery;die();kW
				...
				if(is_null($c1_data[0]['cons'])){
					...
					$resdata['unff'][$i]['current1']=$c1_data[0]['cons'];
				}
				if(is_null($c2_data[0]['cons'])){
					$resdata['unff'][$i]['current2']="NA";
				}else{
					$resdata['unff'][$i]['current2']=$c2_data[0]['cons'];
				}
				if(is_null($c3_data[0]['cons'])){
					$resdata['unff'][$i]['current3']="NA";
				}else{
					$resdata['unff'][$i]['current3']=$c3_data[0]['cons'];
				}
				if(is_null($v1_data[0]['cons'])){
					$resdata['unff'][$i]['voltage1']="NA";
				}else{
					$resdata['unff'][$i]['voltage1']=$v1_data[0]['cons'];
				}
				if(is_null($v2_data[0]['cons'])){
					$resdata['unff'][$i]['voltage2']="NA";
				}else{
					$resdata['unff'][$i]['voltage2']=$v2_data[0]['cons'];
				}
				if(is_null($v3_data[0]['cons'])){
					$resdata['unff'][$i]['voltage3']="NA";
				}else{
					$resdata['unff'][$i]['voltage3']=$v3_data[0]['cons'];
				}
				$resdata['unff'][$i]['model']=$meter_list_unff[$i]['ModelNo'];
				}else{
					$resdata['unff'][$i]['pf']="NA";
					$resdata['unff'][$i]['model']=$meter_list_unff[$i]['ModelNo'];
					$resdata['unff'][$i]['current1']="NA";
		...
		for ($i=0; $i < count($meter_list_unww); $i++) {
			$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
				...
				$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
				 $kwdata = $this->db->query($enquery_kw)->result_array();
				 $resdata['unww'][$i]['live']=$this->get_hardwares_device_data_energymeter_report_undp1($table_name_live,$meter_list_unww[$i]['UtilityName'],$todayDate);;
				if($meter_list_unww[$i]['ModelNo']=='EM6400NG' || $meter_list_unww[$i]['ModelNo']=='EM6436H' || $meter_list_unww[$i]['ModelNo']=='LG5310'){	
		
					
					$enquery_current1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
					// echo $enquery_current1;die();
				 $c1_data = $this->db->query($enquery_current1)->result_array();
		
				 $enquery_current2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
				 $c2_data = $this->db->query($enquery_current2)->result_array();
		
				 $enquery_current3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
				 $c3_data = $this->db->query($enquery_current3)->result_array();
		
		
				 $enquery_volt1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
				 $v1_data = $this->db->query($enquery_volt1)->result_array();
		
				 $enquery_volt2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
				 $v2_data = $this->db->query($enquery_volt2)->result_array();
		
				 $enquery_volt3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unww[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
				 $v3_data = $this->db->query($enquery_volt3)->result_array();
			...
			//echo $enquery;die();kW
				...
				$resdata['unww'][$i]['model']=$meter_list_unww[$i]['ModelNo'];
				}else{
					$resdata['unww'][$i]['pf']="NA";
					$resdata['unww'][$i]['model']=$meter_list_unww[$i]['ModelNo'];
					$resdata['unww'][$i]['current1']="NA";
		...
		for ($i=0; $i < count($meter_list_unsg); $i++) {
			$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
				...
				$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
				 $kwdata = $this->db->query($enquery_kw)->result_array();
				 $resdata['unsg'][$i]['live']=$this->get_hardwares_device_data_energymeter_report_undp1($table_name_live,$meter_list_unsg[$i]['UtilityName'],$todayDate);;
				if($meter_list_unsg[$i]['ModelNo']=='EM6400NG' || $meter_list_unsg[$i]['ModelNo']=='EM6436H' || $meter_list_unsg[$i]['ModelNo']=='LG5310'){	
		
					
					$enquery_current1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
					// echo $enquery_current1;die();
				 $c1_data = $this->db->query($enquery_current1)->result_array();
		
				 $enquery_current2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
				 $c2_data = $this->db->query($enquery_current2)->result_array();
		
				 $enquery_current3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
				 $c3_data = $this->db->query($enquery_current3)->result_array();
		
		
				 $enquery_volt1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
				 $v1_data = $this->db->query($enquery_volt1)->result_array();
		
				 $enquery_volt2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
				 $v2_data = $this->db->query($enquery_volt2)->result_array();
		
				 $enquery_volt3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unsg[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
				 $v3_data = $this->db->query($enquery_volt3)->result_array();
			...
			//echo $enquery;die();kW
				...
				$resdata['unsg'][$i]['model']=$meter_list_unsg[$i]['ModelNo'];
				}else{
					$resdata['unsg'][$i]['pf']="NA";
					$resdata['unsg'][$i]['model']=$meter_list_unsg[$i]['ModelNo'];
					$resdata['unsg'][$i]['current1']="NA";
		...
		for ($i=0; $i < count($meter_list_unab); $i++) {
			$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
				...
				$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
				 $kwdata = $this->db->query($enquery_kw)->result_array();
				 $resdata['unab'][$i]['live']=$this->get_hardwares_device_data_energymeter_report_undp1($table_name_live,$meter_list_unab[$i]['UtilityName'],$todayDate);;
				if($meter_list_unab[$i]['ModelNo']=='EM6400NG' || $meter_list_unab[$i]['ModelNo']=='EM6436H' || $meter_list_unab[$i]['ModelNo']=='LG5310'){	
		
					
					$enquery_current1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
					// echo $enquery_current1;die();
				 $c1_data = $this->db->query($enquery_current1)->result_array();
		
				 $enquery_current2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
				 $c2_data = $this->db->query($enquery_current2)->result_array();
		
				 $enquery_current3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
				 $c3_data = $this->db->query($enquery_current3)->result_array();
		
		
				 $enquery_volt1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
				 $v1_data = $this->db->query($enquery_volt1)->result_array();
		
				 $enquery_volt2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
				 $v2_data = $this->db->query($enquery_volt2)->result_array();
		
				 $enquery_volt3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list_unab[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
				 $v3_data = $this->db->query($enquery_volt3)->result_array();
			...
			//echo $enquery;die();kW
				...
				$resdata['unab'][$i]['model']=$meter_list_unab[$i]['ModelNo'];
				}else{
					$resdata['unab'][$i]['pf']="NA";
					$resdata['unab'][$i]['model']=$meter_list_unab[$i]['ModelNo'];
					$resdata['unab'][$i]['current1']="NA";
	...
	function get_hardwares_device_data_energy_meters_unicef($data){
		...
		$table_name_live=$this->get_table_name_live($station_id);
		// echo $table_name;die();
		$meter_list=$this->get_energymeter_list_unicef($table_name_live,2024000527);
		...
		for ($i=0; $i < count($meter_list); $i++) {
			$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
			//echo $enquery;die();
				$consdata = $this->db->query($enquery)->result_array();
				$today_cons=round($consdata[0]['cons'],2);
				$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
			     $kwdata = $this->db->query($enquery_kw)->result_array();
				 
				if($meter_list[$i]['ModelNo']=='EM6400NG' || $meter_list[$i]['ModelNo']=='EM6436H'){
					$enquery_current1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
					// echo $enquery_current1;die();
			     $c1_data = $this->db->query($enquery_current1)->result_array();

				 $enquery_current2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
			     $c2_data = $this->db->query($enquery_current2)->result_array();

				 $enquery_current3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
			     $c3_data = $this->db->query($enquery_current3)->result_array();


				 $enquery_volt1="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
			     $v1_data = $this->db->query($enquery_volt1)->result_array();

				 $enquery_volt2="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
			     $v2_data = $this->db->query($enquery_volt2)->result_array();

				 $enquery_volt3="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
			     $v3_data = $this->db->query($enquery_volt3)->result_array();

				 $enquery_pf="SELECT Consumption as cons FROM $table_name_live WHERE `UtilityName`='".$meter_list[$i]['UtilityName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='PF'	ORDER BY TxnTime desc limit 1";
			//echo $enquery;die();kW
				...
				$resdata['uncf'][$i]['model']=$meter_list[$i]['ModelNo'];
				}else{
					$resdata['uncf'][$i]['pf']="NA";
					$resdata['uncf'][$i]['model']=$meter_list[$i]['ModelNo'];
					$resdata['uncf'][$i]['current1']="NA";
				 ...
				 $check=$this->chech_energy_consumotion_unicef($meter_list[$i]['UtilityName'],$yesterDay);
				 $yest_consumption=(float)$check[0]['consumption'];
				$monthly_cons_without_today=$this->chech_energy_consumotion_month_unicef($meter_list[$i]['UtilityName'],$firstday,$yesterDay);
				$resdata['uncf'][$i]['meter']=$meter_list[$i]['DashboardName'];
...
function get_hardwares_device_data_energy_meters($data,$data2){
		...
		$table_name=$this->get_table_name($station_id);
		$table_name_live=$this->get_table_name_live($station_id);
		if(isset($table_name_live)){
			$meter_list=$this->get_energymeter_list($table_name_live);
			$yesterDay = date('Y-m-d',strtotime("-1 days"));
			//$yesterDay = "2021-10-18";
			$weekday = date('Y-m-d',strtotime("-7 days"));
			$firstday= date('Y-m-01', strtotime($todayDate));
			...
			$i=0;
			foreach($meter_list as $meters){
				$enquery="SELECT SUM(Consumption) as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kWh'	ORDER BY TxnTime";
				//echo $enquery;die();
					$consdata = $this->db->query($enquery)->result_array();
				$today_cons=round($consdata[0]['cons'],2);
					$enquery_kw="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='kW'	ORDER BY TxnTime desc limit 1";
					 $kwdata = $this->db->query($enquery_kw)->result_array();
	
					 $enquery_current1="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_1'	ORDER BY TxnTime desc limit 1";
					 $c1_data = $this->db->query($enquery_current1)->result_array();
	
					 $enquery_current2="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_2'	ORDER BY TxnTime desc limit 1";
					 $c2_data = $this->db->query($enquery_current2)->result_array();
	
					 $enquery_current3="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Current_3'	ORDER BY TxnTime desc limit 1";
					 $c3_data = $this->db->query($enquery_current3)->result_array();
	
	
					 $enquery_volt1="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_1'	ORDER BY TxnTime desc limit 1";
					 $v1_data = $this->db->query($enquery_volt1)->result_array();
	
					 $enquery_volt2="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_2'	ORDER BY TxnTime desc limit 1";
					 $v2_data = $this->db->query($enquery_volt2)->result_array();
	
					 $enquery_volt3="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Voltage_3'	ORDER BY TxnTime desc limit 1";
					 $v3_data = $this->db->query($enquery_volt3)->result_array();
	
	
					 $enquery_kva="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='KVA'	ORDER BY TxnTime desc limit 1";
					 $kva_data = $this->db->query($enquery_kva)->result_array();
	
					 $enquery_kvah="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='KVAH'	ORDER BY TxnTime desc limit 1";
					...
					$enquery_pf="SELECT Consumption as cons FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='PF'	ORDER BY TxnTime desc limit 1";
				//echo $enquery;die();kW
	...
	function get_hardwares_device_data_waterlevelmeter_report($data,$fromdate,$todate,$fromtime,$totime){
		...
		if($fromtime=='NA'){
			if($locationName=='Domestic Tank-B' && $todayDate==$todate){
				$querywaterlevel="SELECT round(`CurReading`*".$multiplier."/1000,2) as level,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE `TxnDate`  BETWEEN '".$fromdate."' AND '".$todate."' AND `StationId`='".$station_id."' AND `UtilityName`='".$utilityName."' AND `LocationName`='".$locationName."' ORDER BY TxnDate ASC,TxnTime ASC";
			}else{
				$querywaterlevel="SELECT round(`CurReading`*".$multiplier."/1000,2) as level,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE `TxnDate`  BETWEEN '".$fromdate."' AND '".$todate."' AND `StationId`='".$station_id."' AND `UtilityName`='".$utilityName."' AND `LocationName`='".$locationName."' ORDER BY TxnDate ASC,TxnTime ASC";
			}
			...
			if($fromtime=='Select FromTime'){
				...
				for($t=0;$t<count($datesarray);$t++){
					...
					if($datesarray[$t]>=date('Y-m-d')){
						$querywaterlevel="SELECT round(`CurReading`*".$multiplier."/1000,2) as level,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `UtilityName`='".$utilityName."' AND `LocationName`='".$locationName."' ORDER BY TxnDate ASC,TxnTime ASC";
					...
					if(count($check)==1){
						...
						$querywaterlevel="SELECT round(`CurReading`*".$multiplier."/1000,2) as level,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `UtilityName`='".$utilityName."' AND `LocationName`='".$locationName."' ORDER BY TxnDate ASC,TxnTime ASC";
						$datawaterlevel = $this->db->query($querywaterlevel)->result();
						$water_data=array_merge($water_data,$datawaterlevel);
						// $resdata['meter']=$dashboardName;
						// $resdata['leveldata']=$datawaterlevel;
				...
				$querywaterlevel="SELECT round(`CurReading`*".$multiplier."/1000,2) as level,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name WHERE `TxnDate`  BETWEEN '".$fromdate."' AND '".$todate."' AND `StationId`='".$station_id."' AND `UtilityName`='".$utilityName."' AND `LocationName`='".$locationName."' AND TxnTime BETWEEN '".$fromtime."' AND '".$totime."' ORDER BY TxnDate ASC,TxnTime ASC";
...
//echo json_encode($resdata);die();
   ...
   function get_hardwares_device_data_waterlevelmeter_mum_report($data,$fromdate,$todate){
	...
	$meter_list=$this->get_meters_name1($table_name);
			...
			if(count($check)==1){
				...
				if($datesarray[$t]>=date('Y-m-d')){
					if($meters['LocationName']=='Dom. Water Sump'){
						$querywaterlevel="SELECT IF(CurReading*".$meters['multiplier']."/1000>200, round(CurReading*".$meters['multiplier']."/1000-CurReading*0.05*".$meters['multiplier']."/1000,2), round(CurReading*".$meters['multiplier']."/1000-CurReading*0.25*".$meters['multiplier']."/1000,2)) as level,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `UtilityName`='".$utilityName."' AND `LocationName`='".$meters['LocationName']."' ORDER BY TxnDate ASC,TxnTime ASC";
						
					}else{
						$querywaterlevel="SELECT round(CurReading*".$meters['multiplier']."/1000,2) as level,concat(`TxnDate`,' ',`TxnTime`)as time FROM $table_name_live WHERE TxnDate = '".$datesarray[$t]."' AND `StationId`='".$station_id."' AND `UtilityName`='".$utilityName."' AND `LocationName`='".$meters['LocationName']."' ORDER BY TxnDate ASC,TxnTime ASC";
					}
					...
					//$datawaterlevel = $this->db->query($querywaterlevel)->result();
					//$water_data=array_merge($water_data,$array());
					// $resdata['meter']=$dashboardName;
					// $resdata['leveldata']=$datawaterlevel;
					// $water_level_array=array(
					// 	'utility_name'=>$utilityName,
...
//  echo json_encode($resdata);die();
	...
	function check_firepump_running($lineConnected, $utilityName, $reportDate, $pumpName, $serialNo)
{
    return $this->db
        ->select('*')
        ->from('firepump_running_report_tbl')
        ->where('utility_name', $utilityName)
        ->where('line_connected', $lineConnected)
        ->where('pump_name', $pumpName)
        ->where('meter_serial', $serialNo)
...
private function get_sum_consumption($tableName, $where = array())
{
    $row = $this->db
        ->select('SUM(Consumption) AS total', false)
        ->from($tableName)
        ->where($where)
        ->get()
        ->row_array();
...
private function get_latest_reading($tableName, $where = array(), $column = 'CurReading')
{
	// echo $tableName;die();
    $row = $this->db
        ->select($column)
        ->from($tableName)
        ->where($where)
        ->order_by('TxnTime', 'DESC')
        ->limit(1)
        ->get()
        ->row_array();
// echo "ll:".$this->db->last_query($row);die(); 
    return isset($row[$column]) ? $row[$column] : null;
}

private function get_first_reading($tableName, $where = array(), $column = 'CurReading')
{
    $row = $this->db
        ->select($column)
        ->from($tableName)
        ->where($where)
        ->order_by('TxnTime', 'ASC')
        ->limit(1)
        ->get()
        ->row_array();
...
private function get_date_range($fromDate, $toDate)
{
    $dates = array();
...
private function get_firepump_runtime($tableName, $tableNameLive, $stationId, $utilityName, $lineConnected, $meterSerial, $pumpName, $todayDate, $yesterDay, $weekDates, $monthDates)
{
    $today = $this->get_sum_consumption($tableNameLive, array(
    ...
    $cachedYesterday = $this->check_firepump_running($lineConnected, $utilityName, $yesterDay, $pumpName, $meterSerial);

    $yesterday = !empty($cachedYesterday)
        ? (float) $cachedYesterday['running_min2']
        : $this->get_sum_consumption($tableName, array(
            'UtilityName'   => $utilityName,
            'LineConnected' => $lineConnected,
            'MeterSerial'   => $meterSerial,
            'StationId'     => $stationId,
    ...
    foreach ($weekDates as $date) {
        $cached = $this->check_firepump_running($lineConnected, $utilityName, $date, $pumpName, $meterSerial);
        if (!empty($cached)) {
            $weekly += (float) $cached['running_min2'];
        } else {
            $weekly += $this->get_sum_consumption($tableName, array(
                'UtilityName'   => $utilityName,
                'LineConnected' => $lineConnected,
                'MeterSerial'   => $meterSerial,
                'StationId'     => $stationId,
    ...
    foreach ($monthDates as $date) {
        $cached = $this->check_firepump_running($lineConnected, $utilityName, $date, $pumpName, $meterSerial);
        if (!empty($cached)) {
            $monthly += (float) $cached['running_min2'];
        } else {
            $monthly += $this->get_sum_consumption($tableName, array(
                'UtilityName'   => $utilityName,
                'LineConnected' => $lineConnected,
                'MeterSerial'   => $meterSerial,
                'StationId'     => $stationId,
...
private function format_firepump_status($runReading, $switchReading = null)
{
    $data = array(
        'datar'          => is_null($runReading) ? 'OFFLINE' : 'ONLINE',
        'datas'          => is_null($switchReading) ? 'OFFLINE' : 'ONLINE',
        'running_status' => !is_null($runReading) ? ((float) $runReading != 0) : 'OFFLINE',
        'switch_status'  => !is_null($switchReading) ? ((float) $switchReading == 1) : 'OFFLINE'
    );

    return $data;
}
function get_hardwares_device_data_firepump1($data){
	$station_id    = $data['station_id'];
    $table_name_live    = $this->get_table_name_live($station_id);
	$table_name    = $this->get_table_name($station_id);
    $dashboardName = $data['dashboard_name'];
	...
	$configs_old = array(
    array(
        'meter'           => 'Panel Power Supply',
        'utility'         => 'Old Fire Pump',
        'run_status_line' => 'Mains On',
        'switch_line'     => 'Mains On',
        'runtime_line'    => 'J_Pump Auto RHT',
        'serial'          => '0071'
    ),array(
        'meter'           => 'Jockey Pump',
        'utility'         => 'Old Fire Pump',
        'run_status_line' => 'Jockey Pump On',
        'switch_line'     => 'Jockey Pump Auto',
        'runtime_line'    => 'J_Pump Auto RHT',
        'serial'          => '0071'
    ),
    array(
        'meter'           => 'Main Pump',
        'utility'         => 'Old Fire Pump',
        'run_status_line' => 'Main Pump On',
        'switch_line'     => 'Auto Mode',
        'runtime_line'    => 'Main Pump RHT',
        'serial'          => '0071'
    ),
    array(
        'meter'           => 'Diesel Engine Driven Pump',
        'utility'         => 'Old Fire Pump',
        'run_status_line' => 'DG ON',
        'switch_line'     => 'DG Auto',
        'runtime_line'    => 'DG On RHT',
        'serial'          => '0071'
    )
);
foreach ($configs_old as $i => $cfg) {
    $runReading = $this->get_latest_reading($table_name_live, array(
        'UtilityName'   => $cfg['utility'],
        'LineConnected' => $cfg['run_status_line'],
        'MeterSerial'   => $cfg['serial'],
        'StationId'     => $station_id
    ));
// echo json_encode($runReading);die();
    $switchReading = $this->get_latest_reading($table_name_live, array(
        'UtilityName'   => $cfg['utility'],
        'LineConnected' => $cfg['switch_line'],
        'MeterSerial'   => $cfg['serial'],
        'StationId'     => $station_id
    ));

    $runtime = $this->get_firepump_runtime(
        $table_name,
        $table_name_live,
        $station_id,
        $cfg['utility'],
    ...
    $status = $this->format_firepump_status($runReading, $switchReading);

    $resultArray['run_data'][$i] = array(
        'meter'                  => $cfg['meter'],
        'datar'                  => $status['datar'],
        'datas'                  => $status['datas'],
        'running_status'         => $status['running_status'],
        'switch_status'          => $status['switch_status'],
        'today_running_hours'    => $this->secondsToTime($runtime['today'] * 60),
        'yesterday_running_hours'=> $this->secondsToTime($runtime['yesterday'] * 60),
        'lastweek_running_hours' => $this->secondsToTime($runtime['weekly'] * 60),
        'monthly_running_hours'  => $this->secondsToTime_month($runtime['monthly'] * 60)
    );
...
echo json_encode($resultArray);die();
	...
	function check_firepump_running1($lineconnected,$utilityName,$datesarray,$pump_name,$serial_no){
		$this->db->select('*');
	...
	function get_hardwares_device_data_firepump($data){
		...
		//$firepumpList=$this->get_firepumplist($meterserial,$table_name_live,$utilityName);
		...
		if($meterserial=='71'){
			...
			foreach($firepumpList as $list){
				if($list['fire_pump_name']=='Panel Power Supply'){
					...
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name_live where UtilityName='Old Fire Pump' and LineConnected='J_Pump Auto RHT' and MeterSerial='0071'  and StationId='".$station_id."' and TxnDate='".$todayDate."' ";
				//    echo $runn_today;die();
					...
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name_live where UtilityName='Old Fire Pump' and LineConnected='Main Pump RHT' and MeterSerial='0071'  and StationId='".$station_id."' and TxnDate='".$todayDate."' ";
				   //echo $pressure;die();
					$datarun_today = $this->db->query($runn_today)->result_array();
					$today_runn=(int)$datarun_today[0]['run'];
					...
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name_live where UtilityName='Old Fire Pump' and LineConnected='DG On RHT' and MeterSerial='0071'  and StationId='".$station_id."' and TxnDate='".$todayDate."' ";
				   //echo $pressure;die();
					$datarun_today = $this->db->query($runn_today)->result_array();
					$today_runn=(int)$datarun_today[0]['run'];
		   ...
		   echo json_encode($resultArray['run_data']);die();
		   		$pressure="SELECT round(CurReading*Multiplier*"."0.77,2) as pressure,TxnTime FROM $table_name_live where UtilityName='Old Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0068' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate='".$todayDate."'  ORDER BY TxnTime asc ";
				//    echo $pressure;die();
				$pressuredata = $this->db->query($pressure)->result_array();
				$resultArray['pressure_data']=$pressuredata;
				$resultArray['dg_data']=$this->get_hardwares_device_data_dg_firepump1($table_name_live);
		}else{
			...
			$resultArray['pressure_data']=$pressuredata;
			$resultArray['dg_data']=$this->get_hardwares_device_data_dg_firepump($table_name_live);
       ...
       // $todayDate='2020-07-11';
		...
		$resultArray=array();
			...
			foreach($hydrolist as $list){
				...
				$todayqueryConsu="SELECT SUM(Consumption) as cons FROM $table_name_live where LineConnected='".$runconnect."'  and StationId='".$station_id."' and TxnDate='".$todayDate."' and MeterSerial='0061'  ";
				...
				$todayquery="SELECT Consumption FROM $table_name_live where LineConnected='".$runconnect."'  and StationId='".$station_id."' and TxnDate='".$todayDate."' and MeterSerial='0061'  ORDER BY TxnTime desc limit 1";
		   ...
		   $pressure="SELECT round(CurReading*Multiplier*"."0.49,2) as pressure,TxnTime FROM $table_name_live where UtilityName='PressureMonitor' and LocationName='Hyd.Pneu.System' and StationId='".$station_id."' and TxnDate='".$todayDate."'  ORDER BY TxnTime asc ";
		//  echo  $pressure;die(); 
		   $pressuredata = $this->db->query($pressure)->result_array();		  
		   $resultArray['pressure_data']=$pressuredata;
		   return $resultArray;
...
// echo json_encode($firepumpList);die();
	...
	function get_energymeter_list_undp($station_id){
	...
	function get_energymeter_list_cv_undp($station_id){
	...
	function get_energymeter_list_unicef($table_name,$station_id){
	...
	function get_hardwares_device_data_switch_status($data){
		...
		$meter_list=$this->get_meter_list($table_name_live);
		$i=0;
		...
		foreach($meter_list as $meters){
			if (in_array($meters['LocationName'], $mcb)){
				...
				$mcboff="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Mcb-Off' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
				$mcboffdata = $this->db->query($mcboff)->result_array();

				$mcbon="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Mcb On' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
				$mcbondata = $this->db->query($mcbon)->result_array();
				
				$mcbtrip="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Mcb Trip' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
				$mcbtripdata = $this->db->query($mcbtrip)->result_array();
				
				$yphase="SELECT `CurReading` FROM $table_name_live WHERE  `TxnDate`='".$todayDate."' AND `UtilityName`='".$utilityName."' AND LineConnected='Y-Phase' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
				$yphasedata = $this->db->query($yphase)->result_array();
				//echo $yphase;die();
				$rphase="SELECT `CurReading` FROM $table_name_live WHERE  `TxnDate`='".$todayDate."' AND `UtilityName`='".$utilityName."' AND LineConnected='R-Phase' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
				$rphasedata = $this->db->query($rphase)->result_array();
				//echo $rphasedata[0]['CurReading'];die();
				$bphase="SELECT `CurReading` FROM $table_name_live WHERE  `TxnDate`='".$todayDate."' AND `UtilityName`='".$utilityName."' AND LineConnected='B-Phase' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
				...
				if($meters['LocationName']=='PDB pump'){
					$pon="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Power Off' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
					
	
					$poff="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Power On' and MeterName='Status Monitor' ORDER BY TxnTime DESC LIMIT 1";
					$poffdata = $this->db->query($poff)->result_array();
					$pondata = $this->db->query($pon)->result_array();
	
					$trip="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Trip' and MeterName='Status Monitor' ORDER BY TxnTime DESC LIMIT 1";
					$tripdata = $this->db->query($trip)->result_array();
				}else{
					$poff="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Power Off' and MeterName='Status Monitor'	ORDER BY TxnTime DESC LIMIT 1";
					$poffdata = $this->db->query($poff)->result_array();
	
					$pon="SELECT `CurReading` FROM $table_name_live WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate`='".$todayDate."' AND LineConnected='Power On' and MeterName='Status Monitor' ORDER BY TxnTime DESC LIMIT 1";
					$pondata = $this->db->query($pon)->result_array();
...
// echo $non_emergency_evng;die();
	...
	function get_hardwares_device_data($data){
		...
		if(isset($table_name)){
			...
			$resdata['status']=$status;
			$resdata['dgname']=$dashboardName;
			$graphdata=$this->Api_data_model->graph_data($hardware_name,$todayDate,$table_name_live,$station_id);
			$resdata['graph']=$graphdata;
...
return $resultArray;
	...
	function get_water_meter_data($date){
		...
		foreach($meter_list as $meters){
					...
					$querywaterlevel="SELECT CurReading  FROM hardware_station_consumption_data_mumbai_live WHERE TxnDate='".$todayDate."' AND `StationId`='".$meters['StationId']."' AND `UtilityName`='".$meters['UtilityName']."' AND `LocationName`='".$meters['LocationName']."' AND TxnTime <= '".$time."'  ORDER BY TxnTime DESC limit 1";
					...
					$querywaterlevelCurrent="SELECT CurReading  FROM hardware_station_consumption_data_mumbai_live WHERE TxnDate='".$todayDate."' AND `StationId`='".$meters['StationId']."' AND `UtilityName`='".$meters['UtilityName']."' AND `LocationName`='".$meters['LocationName']."'    ORDER BY TxnTime DESC limit 1";
		// echo $querywaterlevel."<br>";
