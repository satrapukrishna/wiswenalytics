...
class MonitoringDemo extends CI_Controller
{
	function __construct(){
		parent::__construct();
		$this->load->model('Ambiance_model');
	...
	function getSodexoData(){
		...
		$sodexoData=$this->Ambiance_model->getSodexoReport($this->input->get(),'Toilet02');		
    ...
    function getSupervisorDayData(){
        $supData=$this->Ambiance_model->getSupervisorData($this->input->get(),'Toilet03');
    ...
    //print_r($fbd);die();
        ...
        $FeedbackData=$this->Ambiance_model->getAndUpdateSupervisorData($Fdata);
	...
	function getAndUpdateFeedback(){
		...
		$FeedbackData=$this->Ambiance_model->getAndUpdateFeedbackData($Fdata);
	...
	function getFeedbackData(){
		$gfbd=$this->input->get();
		 ...
		 $GetFeedbackData=$this->Ambiance_model->getFeedbackData($gfbd['tid']);
	...
	function getAndUpdateJanitorData(){
		...
		$FeedbackData=$this->Ambiance_model->getAndUpdateFeedbackData($Fdata);
		$JanitorData=$this->Ambiance_model->updateJanitorData($Jdata);
...
function checkData($date,$stationId){
    //echo $date;
    $fuelData = $this->Ambiance_model->getsodexoDataWithDate($date,$stationId);
    return $fuelData;
//echo count($fuelData);
    //print_r($fuelData);
    ...
    function getSodexoReport(){
        ...
        $dateto=$data['todate'];
        //echo $date;die();
        $fuelData = $this->Ambiance_model->getsodexoData($this->input->get());        
    ...
    function getSodexoReportLive(){
        ...
        $date=date("Y-m-d");
             ...
             $fuelData = $this->Ambiance_model->getsodexoDataLive();        
          echo json_encode($fuelData);
         
       
    }
    function getSodexoReportCron(){  
        $fuelData = $this->Ambiance_model->getsodexoDataCron();  
        echo json_encode($fuelData);  
    }
    function getOudorLevelReport(){
        $data = $this->Ambiance_model->getOudorLevel($this->input->get());  
        echo json_encode($data); 
    }
    function getOudorLevelReportLiveDashboard(){
        $data = $this->Ambiance_model->getOudorLevelLiveDashboard();  
        echo json_encode($data); 
    }
    
    function getJanitorReport(){
        $data = $this->Ambiance_model->getJanitorData($this->input->get());  
        echo json_encode($data); 
    }
    function getJanitorReportLive(){
        $data = $this->Ambiance_model->getJanitorDataLiveDashboard();  
        echo json_encode($data); 
    }
    function getFeedbackReport(){
        //echo "kk";
        $data = $this->Ambiance_model->getFeedbackDataDay($this->input->get());  
        echo json_encode($data); 
    }
      function getFeedbackReportLive(){
        //echo "kk";
        $data = $this->Ambiance_model->getFeedbackDataDayLive($this->input->get());  
        echo json_encode($data); 
    }
    function sendMailReport(){
        $date = date('Y-m-d',strtotime("-1 days"));
        $odourRightLevels=$this->Ambiance_model->getOdourRightLevelsMail($date);
        $odourLeftLevels=$this->Ambiance_model->getOdourLeftLevelsMail($date);
        $mailData = $this->Ambiance_model->getSodexoReportMail($date); 
        $footfallData = $this->Ambiance_model->getsodexoDataMail($date);
        $odourReadingLeft=$this->Ambiance_model->getOdourLeftDataMail($date);
        $odourReadingRight=$this->Ambiance_model->getOdourRightDataMail($date); 
