...
class Fire_model extends CI_Model{
    ...
    function getChillerData(){
        ...
        $query="SELECT TxnDate,FromTime,ToTime,PrvReading,CurReading  FROM app_device_station_consumtion_cbre where UtilityName='Chiller Status_Chiller_4' and TxnDate='".$todayDate."' and CurReading=1";
        $data = $this->db->query($query)->result(); 

        $chillerdata= array();
        $minutes = count($data)*11;
    ...
    function getCoolingData(){
        ...
        $dataStatus = $this->db->query($queryStatus)->result();
        $query="SELECT TxnDate,FromTime,ToTime,PrvReading,CurReading  FROM app_device_station_consumtion_cbre where UtilityName='CT Fan Status_CT Fan' and TxnDate='".$todayDate."' and CurReading=1";
        $data = $this->db->query($query)->result(); 
        ...
        return $fulldata;
            ...
            if($data['fromtime']!='Select Hours From'){
                ...
                for ($k=0; $k < count($datesarray); $k++) { 
                     ...
                     $query1 = "SELECT SUM(CurReading)*11 as Consumption  FROM app_device_station_consumtion_cbre where UtilityName='Chiller Status_Chiller_4' AND TxnDate='".$datesarray[$k]."'   AND FromTime BETWEEN ".$fromtime." AND ".$totime." AND ToTime BETWEEN ".$fromtime." AND ".$totime."";
                    $data1 = $this->db->query($query1)->result();
                ...
                for ($k=0; $k < count($datesarray); $k++) {
                    $query1 = "SELECT SUM(CurReading)*11 as Consumption  FROM app_device_station_consumtion_cbre where UtilityName='Chiller Status_Chiller_4' AND TxnDate='".$datesarray[$k]."' ";
                    $data1 = $this->db->query($query1)->result();
                    $fulldata[$k]['Time']=$datesarray[$k];
                    $fulldata[$k]['Meter']='Chiller_4';
                    $fulldata[$k]['runninghrs']=$data1[0]->Consumption;
...
function getFirepumpGraphData($data){
        ...
        if($data['fromdate'] == $data['todate'] && $data['fromtime']=='Select Hours From'){
         ...
         return $fulldata;
            ...
            $query1 = "SELECT SUM(Consumption) as Consumption  FROM app_device_station_consumtion_cbre where UtilityName='Jockey Running Time_Fire Pump house' AND TxnDate=".$date."   AND FromTime BETWEEN '".$from."' AND '".$to."' AND ToTime BETWEEN '".$from."' AND '".$to."'";
                $data1 = $this->db->query($query1)->result();
                    $fulldata[$l][$i]['Time']=$from." To ".$to;
                      $fulldata[$l][$i]['meter']='Jockey Running Time_Fire Pump house';
                    $fulldata[$l][$i]['runninghrs']=$data1[0]->Consumption;
            ...
            if($data['fromtime']!='Select Hours From'){
                ...
                for ($i1=$date_from; $i1<=$date_to; $i1+=86400) {
                 ...
                 for ($l=0; $l <count($locations) ; $l++) {
                     ...
                     $query1 = "SELECT SUM(Consumption) as Consumption  FROM app_device_station_consumtion_cbre where UtilityName='Jockey Running Time_Fire Pump house' AND TxnDate='".$datesarray[$k]."'   AND FromTime BETWEEN ".$fromtime." AND ".$totime." AND ToTime BETWEEN ".$fromtime." AND ".$totime."";
                    $data1 = $this->db->query($query1)->result();
                ...
                for ($l=0; $l <count($locations) ; $l++) {
                    for ($k=0; $k < count($datesarray); $k++) {
                    $query1 = "SELECT SUM(Consumption) as Consumption  FROM app_device_station_consumtion_cbre where UtilityName='Jockey Running Time_Fire Pump house' AND TxnDate='".$datesarray[$k]."' ";
                    $data1 = $this->db->query($query1)->result();
                    $fulldata[$l][$k]['Time']=$datesarray[$k];
                    $fulldata[$l][$k]['meter']='Jockey Running Time_Fire Pump house';
                    $fulldata[$l][$k]['runninghrs']=$data1[0]->Consumption;
...
function firePumpRunnDataAll($data){
    ...
    //echo   ; 
        ...
        $firepump = "SELECT SUM(Consumption) as Consumption  FROM app_device_station_consumtion_cbre where UtilityName='Jockey Running Time_Fire Pump house' AND TxnDate='".$datesarray[$k]."'  ";
    ...
    $fulldata[$k][$i]['date']=$datesarray[$k];                                
    $fulldata[$k][$i]['RunningHours']=$runn[0]->Consumption*11;
    }
