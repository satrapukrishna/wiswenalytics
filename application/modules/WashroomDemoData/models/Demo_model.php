...
function getWaterLeakConsolidate($fromdate,$todate,$stationid,$table_name,$location){
    ...
    function get_location_dropdown() {
        ...
        $this->db->from('warangal_washroom_branches_demo');
        $this->db->order_by('id');
        $res = $this->db->get()->result_array();
        $items = array();
        foreach ($res as $row){
...
function getFeedbackDataDayLivecreport($date,$location,$branch,$stationid,$table_name){
    ...
    function getSodexoReport1($date,$location,$branch,$stationid,$table,$built_since,$managed_by,$operator_name,$operator_contact,$time){
        $table=$table."_live1";
        $result=array();
