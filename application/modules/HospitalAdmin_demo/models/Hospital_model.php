...
class Hospital_model extends CI_Model{
    ...
    function insert_category($data) {
        if (count($data) > 0) {
            $res = $this->db->insert('hospital_departments', $data);
            // echo $this->db->last_query() ;die();
            return $res;
        }
        return false;
    }
    function getRows($dep,$subdep){
        $this->db->select('*');
        $this->db->from('hospital_departments');        
		
		$this->db->where('service_department_name',$dep);
		
		$this->db->where('service_sub_department_name',$subdep);	  
    ...
    function update_department($did, $data) {
        if ($did != '') {
            $this->db->where('department_id', $did);
            $page = $this->db->update('hospital_departments', $data);
            return $page;
        }
        return false;
    }
    function delete_department($department_id) {
        ...
        if ($department_id != '') {
            ...
            $page = $this->db->delete('hospital_departments');
            return $page;
        }
    ...
    function delete_employee($emp_id) {
        ...
        if ($emp_id != '') {
            ...
            $page = $this->db->delete('hospital_employees');
            return $page;
        }

        return false;
    }
    function get_departments_byid($clid,$did){
        $this->db->select('*');
        $this->db->from('hospital_departments');        
    ...
    function get_departments($clid){
        // SELECT DISTINCT `service_department_name` FROM `hospital_departments`
        $this->db->distinct('service_department_name');

        $this->db->select('service_department_name');
        $this->db->from('hospital_departments');        
        $res = $this->db->get()->result_array(); 
        $i=0;
        foreach ($res as  $value) {
            // echo $value['service_department_name'];
            $this->db->select('*');
            $this->db->from('hospital_departments');  
            $this->db->where('service_department_name',$value['service_department_name']);      
            $res[$i] = $this->db->get()->result_array(); 
    ...
    function get_department_dropdown() {
        ...
        $items = array();
        foreach ($res as $row){
            $items[$row['service_department_name']] = $row['service_department_name'];
        }
        return $items;
    }
    function get_sub_department($department_id){
		$this->db->select('department_id,service_sub_department_name');
        $this->db->from('hospital_departments');        
        if ($department_id != ''){
            $this->db->where('service_department_name', $department_id);
        }
        //$this->db->where('device_id!=',3);
        //$this->db->where('status',1);
        $this->db->order_by('service_sub_department_name');
        $res = $this->db->get();
        $items = array();
        foreach ($res->result_array() as $row) {
            $items[$row['department_id']] = $row['service_sub_department_name'];
        }
        return $items;
	}
    function insert_employee($data) {
        if (count($data) > 0) {
            $res = $this->db->insert('hospital_employees', $data);
            return $res;
        }
        return false;
    }
    function get_employees($web_options) {
        ...
        foreach ($res as  $value) {
            ...
            $this->db->select('e.*,hc.service_sub_department_name');
        $this->db->from('hospital_employees as e');
        $this->db->join('hospital_departments as hc', 'hc.department_id=e.sub_department','left');
        $this->db->where('department',$value['service_department_name']); 
        if ($search_keyword != '') {
            $this->db->where("(e.name LIKE '%" . $search_keyword . "%' OR e.emp_id LIKE '%". $search_keyword . "%' OR hc.service_sub_department_name LIKE '%". $search_keyword . "%' OR hc.service_department_name LIKE '%" . $search_keyword . "%' OR e.email_id LIKE '%" . $search_keyword . "%')");
        } 
		// if ($status != '') {
		...
		// $this->db->select('service_sub_department_name');
        // $this->db->from('hospital_departments');        
		
		// $this->db->where('department_id',$websites[0]['sub_department']);
		
		// //$this->db->where('service_sub_department_name',$subdep);	  
        // // $this->db->group_by('h.hardware_device');
        // $res_sub = $this->db->get()->result_array();  
        // $websites[0]['sub_department']=$res_sub[0]['service_sub_department_name'];
		//echo $this->db->last_query();exit;
        $this->db->select('count(e.emp_id) as ttl_rows');
        $this->db->from('hospital_employees as e');
        
        if ($search_keyword != '') {
            $this->db->where("(e.name LIKE '%" . $search_keyword . "%' OR e.emp_id LIKE '%" . $search_keyword . "%' OR e.email_id LIKE '%" . $search_keyword . "%')");
        }
		// if ($status != '') {
		// 	$this->db->where('e.status',$status);
		// }
       ...
       // echo $this->db->last_query();exit;
        ...
        $res[$value['service_department_name']]=$websites;
        
        $i++;
        }
		
		$res['rows']=$row->ttl_rows;
        return $res;
    }
                        ...
                        if(count($check)==12){
                            ...
                            if($datesarray[$k]>=date('Y-m-d')){
                                ...
                                for($j=0;$j<count($times2data); $j++){
                                    ...
                                    $w2="SELECT (CurReading-Consumption) as con FROM $table WHERE LineConnected='Water Level' AND TxnDate ='".$datesarray[$k]."' and TxnTime = '".$times2data[$j]['TxnTime']."' AND (CurReading-Consumption)>0";
                                ...
                                $fulldata[$k]['date']=$datesarray[$k];
                                $fulldata[$k]['leak']=(round($leak-$error_correction,2));
                            ...
                            $leak=$water1leak+$water2leak;
                            echo $leak."------".$lw1."---".$lw2;die();
                                $fulldata[$k]['date']=$datesarray[$k];
                                $fulldata[$k]['leak']=(round($leak-$error_correction,2));
        
                                    $water_leak_query=array(
                                    'location'=>$data['location'],
                                    'leakage'=>$fulldata[$k]['leak'],
                                    'date'=>$datesarray[$k],
                                    'stationid'=>$stationid            
                                );
    ...
    function getJpData($date){
        ...
        $querywaterlevel="SELECT TxnTime as ToTime,CurReading FROM hardware_station_consumption_data_wr_jpnagar where StationId=2022000113 and LineConnected='Water Level' and TxnDate=".$date." order by TxnTime ASC";
        ...
        $result[0]['percent']=round(($waterlevel/1500)*100);
    ...
    function getSodexoReport($data,$toiletId){
       ...
       // $date = "'2020-11-04'";
        ...
        $querywaterlevel="SELECT TxnTime as ToTime,CurReading FROM hardware_station_consumption_data_wr_collector where StationId=2022000112 and LineConnected='Water Level' and TxnDate=".$date." order by TxnTime ASC";
        $datawaterlevel = $this->db->query($querywaterlevel)->result_array();
        $waterlevel=$datawaterlevel[0]['CurReading'];
            ...
            // echo  $data_total[0]['Consumption'];die();
                       ...
                       if(isset($data_female[0]['Consumption'])){
                        ...
                        if($tfoot_male==0){
                            ...
                            $fulldata[$i]['footfall_male_percent']=round(($footfall/$tfoot_male)*100,2);
                        }
                       if($tfoot_female==0){
                        $fulldata[$i]['footfall_female_percent']=0;
                       }else{
                        $fulldata[$i]['footfall_female_percent']=round(($footfall_female/$tfoot_female)*100,2);
                       }
