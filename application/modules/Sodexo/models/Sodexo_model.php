<?php
class Sodexo_model extends CI_Model{
    ...
    function getFootfallHourly($data){
        ...
        $query1="SELECT (SELECT CurReading FROM app_device_station_consumtion_sodexo WHERE TxnDate=".$date." and
             StationId='2019000077' and UtilityName='FootFall Male_RestRoom' ORDER BY Id LIMIT 1) as 'first',
            (SELECT CurReading FROM app_device_station_consumtion_sodexo WHERE TxnDate=".$date." and
    ...
    function getSodexoDataReport($toiletId){
        ...
        //echo $query1;die();
        $query="select id, LocationwithUtility,UomScale,TxnDate, FromTime, ToTime, Consumption,CurReading,StationName from (select id, LocationwithUtility,TxnDate, FromTime, ToTime, Consumption ,UomScale,CurReading,StationName from protech_bms.app_device_station_consumtion_sodexo where
           ...
           $stname =explode("-",$data[$i]->StationName);
            if($data[$i]->LocationwithUtility=='FootFall Male_RestRoom'){
                ...
                $result[0]['TxnDate']=$data[$i]->TxnDate;
                    ...
                    $result[0]['footfallcountMale']=0;
                    $result[0]['TxnDate']=$date ;
                    $result[0]['StationName']='Sodexo Office Jubilee Hills';
                    $result[0]['ToTime']=0;
                    $result[0]['OdourMale']=0;
        }
...
//$date = "'2019-12-09'";
    ...
    if($data['fromdate'] == $data['todate'] && $data['fromtime']=='Select Hours From')
    {
        ...
        { 
            ...
            $query="select Consumption FROM protech_bms.app_device_station_consumtion_sodexo where StationId=2019000077 and TxnDate=".$date." and LocationWithUtility='FootFall Male_RestRoom' AND FromTime BETWEEN '".$from."' AND '".$to."' AND ToTime BETWEEN '".$from."' AND '".$to."'";
            $data = $this->db->query($query)->result();
            //echo $query;die();
            $m=0;
