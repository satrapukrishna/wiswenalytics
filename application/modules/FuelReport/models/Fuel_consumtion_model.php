...
function getFuelAddedData($data)
{
    ...
    { 
        ...
        $meterList1=array($metersnames[2],$metersnames[5],$metersnames[3],$metersnames[1],$metersnames[4]);
        $meterList = array();
        for ($i=0; $i < count($meterList1); $i++) { 
        ...
        { 
            ...
            $fulldata[$k][$i]['FuelAdded']=round($datafueladd[0]->fueladd,2);
        }
    ...
    if($data['fromdate'] == $data['todate'] && $data['fromtime']=='Select Hours From')
    {
        ...
        { 
            ...
            else
            {
                ...
                $dataConsumelast = $this->db->query($fuellast)->result();
                $datafueladd = $this->db->query($fueladd)->result();
                $fconsumed_hari=$dataConsumefirst[0]->first-$dataConsumelast[0]->last;
                $fuelconsume=abs((abs($dataConsumefirst[0]->first-$dataConsumelast[0]->last))-$datafueladd[0]->fueladd);

                $fulldata[$i]['meter']=$meter;
                $fulldata[$i]['Time']=$from." To ".$to;
                // echo "timings:".$fulldata[$i]['Time'];
                // echo "  "; echo "hari".$fconsumed_hari. " ".$fuelconsume;echo "<br>";
                // echo "timings:".$fulldata[$i]['Time'];echo "  ";
                // echo "fuelconsumed:".$fuelconsume;echo "<br>";
                $fulldata[$i]['runninghrs']=$s;
        ...
        else
        {
            ...
            { 
                ...
                elseif($meter=='Boiler_2')
                {
                    ...
                    $fuellast = "SELECT CurReading as last FROM app_device_station_consumtion where LocationName='Boiler_1' AND TxnDate='".$datesarray[$k]."' AND UtilityName='Fuel_Level' AND FromTime NOT BETWEEN '23:54:00' AND '24:00:00'  ORDER BY id desc limit 2";
                    ...
                    // $fuelconsume=abs((abs($dataConsumefirst[0]->first-$dataConsumelast[0]->last))-$datafueladd[0]->fueladd);
                    // echo $dataConsumefirst[0]->first; echo $dataConsumelast[0]->last;die();
                    if($dataConsumefirst[0]->first-$dataConsumelast[1]->last >=0)
                    {
                        $fuelconsume=abs((abs($dataConsumefirst[0]->first-$dataConsumelast[0]->last))+$datafueladd[0]->fueladd);
                    }
                    else
                    {
                        $fuelconsume=($dataConsumefirst[0]->first-$dataConsumelast[1]->last)+($datafueladd[0]->fueladd);
                        // $fuelconsume=abs($fuelconsume1);
                    }
                    ...
                    if($m>6)
                    {
                        $fulldata[$k]['Fuelconsume']=round($fuelconsume,2);
                        $fulldata[$k]['economy']=round($fuelconsume/($m/60),3);
                        $fulldata[$k]['runninghrs']=$m;
                    }
                    else
                ...
                elseif($meter==='Boiler_4')
                {
                    ...
                    // echo $fueladd4Today;die();
                    // $fueladd4Today1 = "SELECT SUM(Consumption)  as fueladd FROM app_device_station_consumtion where LocationName='Boiler_4B' AND TxnDate='".$datesarray[$k]."'  AND UtilityName='Fuel_Filled' ";
                    $dataConsumefirst4AToday = $this->db->query($fuelfirst4AToday)->result();
                    $dataConsumelast4AToday = $this->db->query($fuellast4AToday)->result();
                    $dataConsumefirst4BToday = $this->db->query($fuelfirst4BToday)->result();
                    $dataConsumelast4BToday = $this->db->query($fuellast4BToday)->result();
                    ...
                    $A4consumeToday=abs(abs($dataConsumefirst4AToday[0]->first-$dataConsumelast4AToday[0]->last));
                    $B4consumeToday=abs(abs($dataConsumefirst4BToday[0]->first-$dataConsumelast4BToday[0]->last));
                    // echo $A4consumeToday;die();
                    $consume4Today=$A4consumeToday+$B4consumeToday;
                    // echo $consume4Today."<br>";die();
                    $fuelconsume4Today=abs($consume4Today-$datafueladdToday[0]->fueladd);
                    //echo $fuelconsume4Today."<br>";
                    // if($datafueladdToday[0]->fueladd==0)
                    // {
                        // $fueladd=$datafueladdToday[0]->fueladd;
                    // }
                    // elseif ($datafueladdToday[0]->fueladd>5) 
                    // {
                    //     $fueladd=$datafueladdToday[0]->fueladd+$datafueladdToday[0]->fueladd;
                    // }
                    // else
                    // {
                    //     $fueladd=$datafueladdToday[0]->fueladd;
                ...
                {                    
                    ...
                    // $fueladd_time = "SELECT FromTime,ToTime FROM app_device_station_consumtion where LocationName='".$meter."' AND TxnDate='".$datesarray[$k]."' AND UtilityName='Fuel_Filled' AND FromTime NOT BETWEEN '23:58:00' AND '24:00:00'  AND Consumption>0 ";
                    ...
                    // echo $fuelfirst;die();
                    $fuellast = "SELECT CurReading as last FROM app_device_station_consumtion where LocationName='".$meter."' AND TxnDate='".$datesarray[$k]."' AND UtilityName='Fuel_Level' AND FromTime NOT BETWEEN '23:54:00' AND '24:00:00'  ORDER BY id desc limit 2";
                    $dataConsumefirst = $this->db->query($fuelfirst)->result();
                    $dataConsumelast = $this->db->query($fuellast)->result();
                   
                    $datafueladd = $this->db->query($fueladd)->result();
                    // $datafueladd_timngs= $this->db->query($fueladd_time)->result();
                    

                    // $fueladdfromtime = $datafueladd_timngs[0]->FromTime;
                    // $fueladdtotime = $datafueladd_timngs[0]->ToTime;

                    // $getFuelLevelsAfterFueladded = "SELECT CurReading,Consumption,PrvReading FROM app_device_station_consumtion where LocationName='".$meter."' AND TxnDate='".$datesarray[$k]."' AND UtilityName='Fuel_Level' AND FromTime NOT BETWEEN '23:54:00' AND '24:00:00'  AND FromTime  BETWEEN '".$fueladdfromtime."' AND '".$fueladdtotime."' ";
                    // $getFuelLevelsAfterFueladded_exec = $this->db->query($getFuelLevelsAfterFueladded)->result();

                    
                    // $curreadingAfterFuelAdded = $getFuelLevelsAfterFueladded_exec[0]->CurReading;
                    ...
                    $fulldata[$k]['Time']=$datesarray[$k];
                    $fulldata[$k]['runninghrs']=$m;
                    $fulldata[$k]['FuelAdded']=round($datafueladd[0]->fueladd);
                    if($m>0)
                    {
                        $fulldata[$k]['Fuelconsume']=round($fuelconsume,2);
                        $fulldata[$k]['economy']=round($fuelconsume/($m/60),3);
