<?php
class Aurush_data_model extends CI_Model{
	...
	function get_hardwares_device_data_energy_meters($data,$fromdate,$todate){
		$station_id=$data['station_id'];
		
		$todayDate=date("Y-m-d");
		//$todayDate="2021-10-15";
		// $table_name='hardware_station_consumption_data_chennai_6thapril_to_12th_april';
		//$table_name='hardware_station_consumption_data_chennai_22marh_to_5thapril2022';
		//$table_name='hardware_station_consumption_data_chennai_bkp_20thjan22';
		// $table_name='hardware_station_consumption_data_chennai_bkp_13thaprlto5thmay22';
		
		$table_name='hardware_station_consumption_data_chennai';
			...
			{ 
				$n=0;
					...
					for ($i=0; $i < 24; $i++) 
						...
						$resdata1[$i]['year']=date('Y', strtotime($datesarray_month[$k]));
						$resdata1[$i]['month']=date('F', strtotime($datesarray_month[$k]));
						$resdata1[$i]['date']=date('d', strtotime($datesarray_month[$k]));
						$resdata1[$i]['hour']=$i+1;
						$h=0;
						foreach($meter_list as $meters){
							$query = "SELECT SUM(Consumption) as cons  FROM $table_name  WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate` = '".$datesarray_month[$k]."' AND LineConnected='kWh' AND TxnTime BETWEEN '".$from."' AND '".$to."' ";
							// echo $queryD;
								$data1 = $this->db->query($query)->result_array();
								if($meters['LocationName']=="AFS project-C"){ 
									$resdata2[$h]['meter']="Container office";
								}else if($meters['LocationName']=="B4 Building"){
									$resdata2[$h]['meter']="A4 Building";
								}else if($meters['LocationName']=="Mains"){
									$resdata2[$h]['meter']="Main I/C (EB)";
									
									
								}else if($meters['LocationName']=="Fire Fighting"){
									$resdata2[$h]['meter']="Fire Pump Panel";
								}else if($meters['LocationName']=='PDB pump'){
									$resdata2[$h]['meter']='PDB Panel';
								}else if($meters['LocationName']=="LDB Pump"){
									$resdata2[$h]['meter']="LDB (Pump room)";
								}else if($meters['LocationName']=="LDB Street"){
									$resdata2[$h]['meter']="LDB (Street light panel)";
								}else{
									$resdata2[$h]['meter']=$meters['LocationName'];
								}

								
								$resdata2[$h]['cons']=round($data1[0]['cons'],2);
								$resdata2[$h]['meter']=$meters['LocationName'];
								$h++;
						}
						$resdata1[$i]['data']=$resdata2;
								
						}
			//$resdata[$k]['date']=$datesarray_month[$k];
			$resdata[$k]=$resdata1;
...
//echo json_encode($resdata);die();
	...
	function get_meters_name_chn($table_name) {
        $this->db->select('');
        $this->db->from('hardware_station_consumption_data_chennai');     
        $this->db->where('LineConnected','Water Level'); 
		$this->db->group_by('LocationName');
        $res = $this->db->get()->result_array(); 
		//echo json_encode($res[0]); die();
		//echo $this->db->last_query();exit; SELECT * FROM `hardware_station_consumption_data_mumbai` WHERE `UtilityName`='Water Monitor' GROUP BY `LocationName`
...
//echo json_encode($resdata);die();
   return $resdata;
	...
	function get_hardwares_device_data_waterlevelmeter($data,$fromdate,$todate){
		...
		$meter_list=$this->get_meters_name_chn($table_name);
			...
			{ 
				$n=0;
					...
					for ($i=0; $i < 24; $i++) 
						...
						$h=0;
						foreach($meter_list as $meters){
							$query = "SELECT `CurReading`  FROM $table_name  WHERE `LocationName`='".$meters['LocationName']."' AND `UtilityName`='".$meters['UtilityName']."' AND `TxnDate` = '".$datesarray_month[$k]."' AND `StationId`='".$meters['StationId']."' AND  TxnTime BETWEEN '".$from."' AND '".$to."' ORDER BY TxnTime DESC LIMIT 1";
							//echo $query;die();
								$data1 = $this->db->query($query)->result_array();
								if($meters['LocationName']=='Fire 3'){
									$waterlevel=$data1[0]['CurReading']/1.137;
								}else{
									$waterlevel=$data1[0]['CurReading'];
								}

								
								//$resdata2[$h]['meter']=$meters['LocationName'];
								
								$resdata2[$h]['level']=round($waterlevel/1000,2);
								$resdata2[$h]['meter']=$meters['LocationName'];
								$h++;
						}
						$resdata1[$i]['data']=$resdata2;
	...
	function get_hardwares_device_data_firepump($data,$fromdate,$todate){
		//print_r($data);die();
		$station_id=$data['station_id'];
		//$table_name='hardware_station_consumption_data_chennai_6thapril_to_12th_april';
		//$table_name='hardware_station_consumption_data_chennai_22marh_to_5thapril2022';
...
// echo json_encode($datesarray_month);die();
			...
			$firepumpList[3]['fire_pump_name']='Jockey Pump_new';
			$firepumpList[4]['fire_pump_name']='Diesel Engine Driven Pump_new';
			$firepumpList[5]['fire_pump_name']='Motor-1';
			$firepumpList[6]['fire_pump_name']='Motor-2';
		for ($k=0; $k < count($datesarray_month); $k++)
			{
				$resultArray=array();
			...
			foreach($firepumpList as $list){
				if($list['fire_pump_name']=='Jockey Pump'){
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name where UtilityName='Old Fire Pump' and LineConnected='J_Pump Auto RHT' and MeterSerial='0071'  and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."' ";
				//    echo $runn_today."<br>";
					$datarun_today = $this->db->query($runn_today)->result_array();
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure FROM $table_name where UtilityName='Old Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0068' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'  ORDER BY TxnTime DESC LIMIT 1";
					// echo $pressure."<br>";
					$pressuredata = $this->db->query($pressure)->result_array();
					$resdata2[$h]['pressure']=$pressuredata[0]['pressure'];
					$resdata2[$h]['run']=$datarun_today[0]['run'];
					
					$resdata2[$h]['meter']='Old Jockey Pump';
					$h++;
				}else if($list['fire_pump_name']=='Main Pump'){
					
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name where UtilityName='Old Fire Pump' and LineConnected='Auto Mode RHT' and MeterSerial='0071'  and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'";
				   //echo $pressure;die();
					$datarun_today = $this->db->query($runn_today)->result_array();
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure FROM $table_name where UtilityName='Old Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0068' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'  ORDER BY TxnTime DESC LIMIT 1";
					$pressuredata = $this->db->query($pressure)->result_array();
					$resdata2[$h]['pressure']=$pressuredata[0]['pressure'];
					$resdata2[$h]['run']=$datarun_today[0]['run'];
					$resdata2[$h]['meter']='Old Main Pump';
					$h++;
				}else if($list['fire_pump_name']=='Diesel Engine Driven Pump'){
					
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name where UtilityName='Old Fire Pump' and LineConnected='DG On RHT' and MeterSerial='0071'  and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'";
				   //echo $pressure;die();
					$datarun_today = $this->db->query($runn_today)->result_array();
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure FROM $table_name where UtilityName='Old Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0068' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'  ORDER BY TxnTime DESC LIMIT 1";
					$pressuredata = $this->db->query($pressure)->result_array();
					$resdata2[$h]['pressure']=$pressuredata[0]['pressure'];
					$resdata2[$h]['run']=$datarun_today[0]['run'];
					$resdata2[$h]['meter']='Old Diesel Engine Driven Pump';
					$h++;
				}else if($list['fire_pump_name']=='Jockey Pump_new'){
					
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name where UtilityName='New Fire Pump' and LineConnected='Jockey Pump RHT' and MeterSerial='0069'  and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'";
				   //echo $pressure;die();
					$datarun_today = $this->db->query($runn_today)->result_array();
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure,TxnTime FROM $table_name where UtilityName='New Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0069' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'  ORDER BY TxnTime DESC LIMIT 1 ";
					$pressuredata = $this->db->query($pressure)->result_array();
					$resdata2[$h]['pressure']=$pressuredata[0]['pressure'];
					$resdata2[$h]['run']=$datarun_today[0]['run'];
					$resdata2[$h]['meter']='New Jockey Pump';
					$h++;
				}else if($list['fire_pump_name']=='Diesel Engine Driven Pump_new'){
					
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name where UtilityName='New Fire Pump' and LineConnected='Engine Run' and MeterSerial='0070'  and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."' ";
				   //echo $pressure;die();
					$datarun_today = $this->db->query($runn_today)->result_array();
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure,TxnTime FROM $table_name where UtilityName='New Fire Pump' and LocationName='Fire Pump House' and MeterSerial='0069' and LineConnected='Pressure' and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'  ORDER BY TxnTime DESC LIMIT 1 ";
					$pressuredata = $this->db->query($pressure)->result_array();
					$resdata2[$h]['pressure']=$pressuredata[0]['pressure'];
					$resdata2[$h]['run']=$datarun_today[0]['run'];
					$resdata2[$h]['meter']='New Diesel Engine Driven Pump_new';
					$h++;
				}
				else if($list['fire_pump_name']=='Motor-1'){
					
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name where LineConnected='Motor-1 RHT' and MeterSerial='0061'  and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."' ";
				   
					$datarun_today = $this->db->query($runn_today)->result_array();
					$pressure="SELECT round(CurReading*Multiplier,2) as pressure,TxnTime FROM $table_name where UtilityName='PressureMonitor' and LocationName='Hyd.Pneu.System' and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."'  ORDER BY TxnTime DESC LIMIT 1 ";

					$pressuredata = $this->db->query($pressure)->result_array();
					$resdata2[$h]['pressure']=$pressuredata[0]['pressure'];
					$resdata2[$h]['run']=$datarun_today[0]['run'];
					$resdata2[$h]['meter']='Hyd.Pneu.System Motor-1';
					$h++;
				}else if($list['fire_pump_name']=='Motor-2'){
					
					$runn_today="SELECT SUM(Consumption) as run FROM $table_name where LineConnected='Motor-2 RHT' and MeterSerial='0061'  and StationId='".$station_id."' and TxnDate='".$datesarray_month[$k]."' AND TxnTime BETWEEN '".$from."' AND '".$to."' ";
					...
					$resdata2[$h]['pressure']=$pressuredata[0]['pressure'];
					$resdata2[$h]['run']=$datarun_today[0]['run'];
					$resdata2[$h]['meter']='Hyd.Pneu.System Motor-2';
					$h++;
				}
	...
	function get_hardwares_device_data_water_meters($data,$fromdate,$todate){
		...
		//$table_name='hardware_station_consumption_data_chennai_bkp_20thjan22';
		$table_name='hardware_station_consumption_data_chennai_bkp_13thaprlto5thmay22';
		$meter_list=$this->get_watermeter_list($table_name);
			...
			{ 
				$n=0;
					...
					for ($i=0; $i < 24; $i++) 
						...
						$h=0;
						foreach($meter_list as $meters){
							$query = "SELECT round(SUM(Consumption)/1000,2) as cons  FROM $table_name  WHERE `LocationName`='".$meters['LocationName']."' AND `TxnDate` = '".$datesarray_month[$k]."' AND LineConnected='Water Flow' AND TxnTime BETWEEN '".$from."' AND '".$to."' ";
							// echo $queryD;
								$data1 = $this->db->query($query)->result_array();
								
								$resdata2[$h]['cons']=$data1[0]['cons'];
								$resdata2[$h]['meter']=$meters['LocationName'];
								$h++;
						}
						$resdata1[$i]['data']=$resdata2;
...
// echo json_encode($firepumpList);die();
	...
	function get_watermeter_list($table_name){
		
		$this->db->select('');
        $this->db->from($table_name);     
        $this->db->where('LineConnected','Water Flow'); 
		$this->db->group_by('LocationName');
        $res = $this->db->get()->result_array(); 
		//echo json_encode($res[0]); die();
		//echo $this->db->last_query();exit;     
