       ...
       // print_r($data);die();
    	...
    	if($data['fromdate'] == $data['todate'] && $data['fromtime']=='Select Hours From'){
    		...
    		for ($i=0; $i < 24; $i++) { 
    			...
    			$query = "SELECT CurReading,Consumption  FROM app_device_station_consumtion where LocationName=".$meter." AND TxnDate=".$date." AND UtilityName='DG_Running_Time' AND FromTime BETWEEN '".$from."' AND '".$to."' AND ToTime BETWEEN '".$from."' AND '".$to."'";
    	...
    	return $fulldata;
    		...
    		if($data['fromtime']!='Select Hours From'){
				...
				for ($k=0; $k < count($datesarray); $k++) { 
					$query =  "SELECT CurReading,Consumption  FROM app_device_station_consumtion where LocationName=".$meter." AND TxnDate='".$datesarray[$k]."' AND UtilityName='DG_Running_Time' AND FromTime BETWEEN ".$fromtime." AND ".$totime." AND ToTime BETWEEN ".$fromtime." AND ".$totime."";
					//echo $query;die();
					$data = $this->db->query($query)->result();
					$m=0;
				    			for ($j=0; $j <count($data) ; $j++) {
				...
				for ($k=0; $k < count($datesarray); $k++) { 
					$query = "SELECT CurReading,Consumption  FROM app_device_station_consumtion where LocationName=".$meter." AND TxnDate='".$datesarray[$k]."' AND UtilityName='DG_Running_Time'";
					$data = $this->db->query($query)->result();
					$m=0;
				    			for ($j=0; $j <count($data) ; $j++) {
				    				$m +=$data[$j]->Consumption;
