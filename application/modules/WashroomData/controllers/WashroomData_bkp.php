    ...
    function getWashroomDashboardData(){
        ...
        if($date==$today){
            foreach ($branches as $row) {
                $wash_results[$i]=$this->Demo_model->getSodexoReport1($date,$row['location'],$row['branch'],$row['stationid'],$row['table_name_live']); 
                $i++;
            }
        }else{
            foreach ($branches as $row) {
                $wash_results[$i]=$this->Demo_model->getSodexoReport1($date,$row['location'],$row['branch'],$row['stationid'],$row['table_name']); 
                $i++;
            }
        }
