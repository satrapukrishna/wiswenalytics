...
class Boiler_model extends CI_Model{
    ...
    function getDashBoilerConsumed($data,$timings){
       // print_r($timings);die();
    	// today calculation
    	if($data['meter']==='Boiler_2'){
            $data['meter']='Boiler_1';
...
//echo $data['meter'];die();
        ...
        if($data['meter']==='Boiler_4A'){
            //today
            
        $fuelfirst = "select sum(CurReading) as first from (select app_device_station_consumtion.CurReading from app_device_station_consumtion where LocationName IN ('Boiler_4A','Boiler_4B') AND TxnDate=".$date." AND UtilityName='Fuel_Level' AND FromTime NOT  BETWEEN '23:58:00' AND '24:00:00' order by app_device_station_consumtion.id desc limit 2) as first";

        $fuellast = "select sum(CurReading) as last from (select app_device_station_consumtion.CurReading from app_device_station_consumtion where LocationName IN ('Boiler_4A','Boiler_4B') AND TxnDate=".$date." AND UtilityName='Fuel_Level' AND FromTime NOT  BETWEEN '23:58:00' AND '24:00:00' order by app_device_station_consumtion.id asc limit 2) as last";
            //lastweek
        $fuelfirst1 = "select sum(CurReading) as first from (select app_device_station_consumtion.CurReading from app_device_station_consumtion where LocationName IN ('Boiler_4A','Boiler_4B')  AND  TxnDate BETWEEN ".$startweekdate." AND ".$endweekdate." AND UtilityName='Fuel_Level' AND FromTime NOT BETWEEN '23:58:00' AND '24:00:00' order by app_device_station_consumtion.id desc limit 2) as first";

        $fuellast1 = "select sum(CurReading) as last from (select app_device_station_consumtion.CurReading from app_device_station_consumtion where LocationName IN ('Boiler_4A','Boiler_4B')  AND  TxnDate BETWEEN ".$startweekdate." AND ".$endweekdate." AND UtilityName='Fuel_Level' AND FromTime NOT BETWEEN '23:58:00' AND '24:00:00' order by app_device_station_consumtion.id asc limit 2) as last";
        //echo $fuelfirst1;die();
            //yesretday
        $fuelfirst2 = "select sum(CurReading) as first from (select app_device_station_consumtion.CurReading from app_device_station_consumtion where LocationName IN ('Boiler_4A','Boiler_4B') AND TxnDate=".$yesterDay." AND UtilityName='Fuel_Level' AND FromTime NOT  BETWEEN '23:58:00' AND '24:00:00' order by app_device_station_consumtion.id desc limit 2) as first";

        $fuellast2 = "select sum(CurReading) as last from (select app_device_station_consumtion.CurReading from app_device_station_consumtion where LocationName IN ('Boiler_4A','Boiler_4B') AND TxnDate=".$yesterDay." AND UtilityName='Fuel_Level' AND FromTime NOT  BETWEEN '23:58:00' AND '24:00:00' order by app_device_station_consumtion.id asc limit 2) as last";
