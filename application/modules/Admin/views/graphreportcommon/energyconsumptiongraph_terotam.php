...
<script>
    <?php if($energydata['day']['unicef'][0][0]['sort']==2){ ?>
        <?php
    for ($k=0; $k < count($energydata['day']['unicef']); $k++) { 
        
        $fulldata=array();
        for($i=0;$i<count($energydata['day']['unicef'][$k]);$i++){ 
            $xdata=array();
            $ydata=array();
            for ($n=0; $n < count($energydata['day']['unicef'][$k][$i]['data']); $n++) { 
                array_push($xdata,$energydata['day']['unicef'][$k][$i]['data'][$n]['date']);
                array_push($ydata,round($energydata['day']['unicef'][$k][$i]['data'][$n]['consumption'],2));
            }
        
            $fulldata[$i]['xdata']=$xdata;
            $fulldata[$i]['ydata']=$ydata;
            $fulldata[$i]['meter']=$energydata['day']['unicef'][$k][$i]['meter'];
            $fulldata[$i]['date']=$energydata['day']['unicef'][$k][$i]['date'];
...
<?php }?>
    ...
    $fulldata=array();
    for($i=0;$i<count($energydata['day']['unicef'][$k]);$i++){ 
    array_push($xdata,$energydata['day']['unicef'][$k][$i]['date']);
    array_push($ydata,round($energydata['day']['unicef'][$k][$i]['consumption'],2));
    
    
    }
    $fulldata['xdata']=$xdata;
    $fulldata['ydata']=$ydata;
    $fulldata['meter']=$energydata['day']['unicef'][$k][0]['meter'];
     ?>
    var data=<?php echo json_encode($fulldata); ?>;
...
Highcharts.chart('energycontainerunicef<?php echo $k; ?>', {
    chart: {
        type: 'column'
    },
