...
class Hardware_category_model extends CI_Model{
	...
	function insert_category($data) {
        if (count($data) > 0) {
            $res = $this->db->insert('hardware_category_demo', $data);
            return $res;
        }
        return false;
    }
	...
	function get_hardware_category_dropdown() {
        ...
        $this->db->from('hardware_category_demo');
    ...
    function get_hardware_category_dropdown_chennai() {
        $this->db->select('category_id,category_name');
        $this->db->from('hardware_category_demo');
        $this->db->where_in('category_id',[5,6,11]);
        $this->db->where('status',1);
        $this->db->order_by('category_id');
        $res = $this->db->get()->result_array();
        $items = array();
    ...
    function get_devices_chennai($category){
        ...
        $this->db->where_in('device_id',[39,40,28,27,19,20,41,26,25]);
        $this->db->where('status',1);
        $this->db->order_by('device_name');
        $res = $this->db->get();
        $items = array();
	...
	function get_hardware_category($web_options) {
		...
		$this->db->select('hc.*');
        $this->db->from('hardware_category_demo as hc');
	...
	function get_hardware_category1($web_options) {
		...
		$this->db->join('hardware_category_demo as hc', 'hcc.category_id=hc.category_id','left');
        if ($search_keyword != '') {
	...
	function get_device($d_id) {
        ...
        $this->db->from('hardware_device1 as hd');
		$this->db->join('hardware_category_demo as hc', 'hd.category_id=hc.category_id','left');
	...
	function update_category($c_id, $data) {
        if ($c_id != '') {
            $this->db->where('category_id', $c_id);
            $page = $this->db->update('hardware_category_demo', $data);
            return $page;
        }
        return false;
    }
	function update_device($d_id, $data) {
        if ($d_id != '') {
            ...
            $page = $this->db->update('hardware_device1', $data);
            return $page;
        }
        return false;
    }
	function delete_category($category_id) {
        ...
        if ($category_id != '') {
            ...
            $page = $this->db->delete('hardware_category_demo');
			
			$this->db->where('category_id', $category_id);
            $page1 = $this->db->delete('hardware_device1');
			
			$this->db->where('hardware_category_demo', $category_id);
	...
	function delete_device($device_id) {
        ...
        if ($device_id != '') {
            ...
            $page = $this->db->delete('hardware_device1');
	...
	function update_status($c_id,$data)
    {
        if(count($data)>0 && $c_id != ''){
            $this->db->where('category_id',$c_id);
            $res = $this->db->update('hardware_category_demo',$data);
            return $res;
        }
        return false;
	...
	function update_status1($d_id,$data)
    {
        if(count($data)>0 && $d_id != ''){
            $this->db->where('device_id',$d_id);
            $res = $this->db->update('hardware_device1',$data);
            return $res;
        }
        return false;
