...
class Login_model extends CI_Model{
    ...
    { 
    	...
    	$query = "SELECT profile.user_id,user_type_id,first_name,last_name
		  FROM protech_bms.`dbo.app_users` users
		  INNER JOIN protech_bms.`dbo.app_users_profile` profile ON users.id = profile.user_id WHERE users.username=".$u." AND users.password=".$p;
    	...
    	if($numberOfRows ==1){
	    	$query1 ="SELECT branch_name,station_id,client_name,station_code
				...
				FROM protech_bms.`dbo.app_users_to_branches` utob 
				RIGHT JOIN protech_bms.`dbo.app_client_branches` acb ON utob.branch_id = acb.id
				INNER JOIN protech_bms.`dbo.app_client` ac ON acb.client_id = ac.id
